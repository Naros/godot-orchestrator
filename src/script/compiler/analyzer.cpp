...
static OScriptParser::DataType make_callable_type(const MethodInfo& p_info) {
    OScriptParser::DataType type;
    type.type_source = OScriptParser::DataType::ANNOTATED_EXPLICIT;
    type.kind = OScriptParser::DataType::BUILTIN;
...
static OScriptParser::DataType make_signal_type(const MethodInfo& p_info) {
    OScriptParser::DataType type;
    type.type_source = OScriptParser::DataType::ANNOTATED_EXPLICIT;
    type.kind = OScriptParser::DataType::BUILTIN;
...
static OScriptParser::DataType make_native_meta_type(const StringName& p_class_name) {
    OScriptParser::DataType type;
    type.type_source = OScriptParser::DataType::ANNOTATED_EXPLICIT;
    type.kind = OScriptParser::DataType::NATIVE;
...
static OScriptParser::DataType make_script_meta_type(const Ref<Script>& p_script) {
    OScriptParser::DataType type;
    type.type_source = OScriptParser::DataType::ANNOTATED_EXPLICIT;
    type.kind = OScriptParser::DataType::SCRIPT;
    type.builtin_type = Variant::OBJECT;
...
static OScriptParser::DataType make_enum_type(const StringName& p_enum_name, const String& p_base_name, const bool p_meta = false) {
    OScriptParser::DataType type;
    type.type_source = OScriptParser::DataType::ANNOTATED_EXPLICIT;
    type.kind = OScriptParser::DataType::ENUM;
...
static OScriptParser::DataType make_class_enum_type(const StringName& p_enum_name, OScriptParser::ClassNode* p_class, const String& p_script_path, bool p_meta = true) {
    OScriptParser::DataType type = make_enum_type(p_enum_name, p_class->fqcn, p_meta);
...
static OScriptParser::DataType make_native_enum_type(const StringName& p_enum_name, const StringName& p_native_class, bool p_meta = true) {
	...
	while (true && native_base != StringName()) {
		...
		native_base = GDE::ClassDB::get_parent_class_nocheck(native_base);
	}

	OScriptParser::DataType type = make_enum_type(p_enum_name, native_base, p_meta);
	if (p_meta) {
	...
	PackedStringArray enum_values = ClassDB::class_get_enum_constants(native_base, p_enum_name, true);
	for (const String& E : enum_values) {
...
static OScriptParser::DataType make_builtin_enum_type(const StringName& p_enum_name, Variant::Type p_type, bool p_meta = true) {
	OScriptParser::DataType type = make_enum_type(p_enum_name, Variant::get_type_name(p_type), p_meta);
	if (p_meta) {
    ...
    Vector<StringName> enum_values = GDE::Variant::get_enumerations_for_enum(p_type, p_enum_name);
	for (const StringName& E : enum_values) {
		type.enum_values[E] = GDE::Variant::get_enum_value(p_type, p_enum_name, E);
	}

	return type;
}

static OScriptParser::DataType make_global_enum_type(const StringName& p_enum_name, const StringName& p_base, bool p_meta = true) {
	OScriptParser::DataType type = make_enum_type(p_enum_name, p_base, p_meta);
	if (p_meta) {
	...
	HashMap<StringName, int64_t> enum_values = GDE::CoreConstants::get_enum_values(type.native_type);
...
static OScriptParser::DataType make_builtin_meta_type(Variant::Type p_type) {
	OScriptParser::DataType type;
	type.type_source = OScriptParser::DataType::ANNOTATED_EXPLICIT;
	type.kind = OScriptParser::DataType::BUILTIN;
...
#ifdef DEBUG_ENABLED
static bool enum_has_value(const OScriptParser::DataType p_type, int64_t p_value) {
...
void OScriptAnalyzer::get_class_node_current_scope_classes(OScriptParser::ClassNode* p_class, List<OScriptParser::ClassNode*>* p_list, OScriptParser::Node* p_source) {
    ...
    if (p_class->base_type.class_type != nullptr) {
        ...
        ensure_cached_external_parser_for_class(p_class->base_type.class_type, p_class, "Trying to fetch classes in the current scope", p_source);
        get_class_node_current_scope_classes(p_class->base_type.class_type, p_list, p_source);
    }
    if (p_class->outer != nullptr) {
        // TODO: 'ensure_cached_external_parser_for_class()' is only necessary because 'resolve_class_inheritance()' is not getting called here.
        ensure_cached_external_parser_for_class(p_class->outer, p_class, "Trying to fetch classes in the current scope", p_source);
        get_class_node_current_scope_classes(p_class->outer, p_list, p_source);
    }
}

void OScriptAnalyzer::decide_suite_type(OScriptParser::Node* p_suite, OScriptParser::Node* p_statement) {
    ...
    switch (p_statement->type) {
        ...
        case OScriptParser::Node::WHILE: {
            ...
            if (p_suite->get_datatype().is_set() && (p_suite->get_datatype() != p_statement->get_datatype())) {
                ...
                p_suite->data_type.kind = OScriptParser::DataType::VARIANT;
                p_suite->data_type.type_source = OScriptParser::DataType::UNDETECTED;
            } else {
                p_suite->set_datatype(p_statement->get_datatype());
                p_suite->data_type.type_source = OScriptParser::DataType::INFERRED;
            }
            break;
        }
        default: {
...
Ref<OScriptParserRef> OScriptAnalyzer::find_cached_external_parser_for_class(const OScriptParser::ClassNode* p_class, const Ref<OScriptParserRef>& p_dependant_parser) {
    if (p_dependant_parser.is_null()) {
        return nullptr;
    }

    if (HashMap<const OScriptParser::ClassNode*, Ref<OScriptParserRef>>::Iterator E = p_dependant_parser->get_analyzer()->external_class_parser_cache.find(p_class)) {
        if (E->value.is_valid()) {
            // Silently ensure it's parsed.
            E->value->raise_status(OScriptParserRef::PARSED);
    ...
    // Silently ensure it's parsed.
    p_dependant_parser->raise_status(OScriptParserRef::PARSED);
    return find_cached_external_parser_for_class(p_class, p_dependant_parser->get_parser());
}

Ref<OScriptParserRef> OScriptAnalyzer::find_cached_external_parser_for_class(const OScriptParser::ClassNode* p_class, OScriptParser* p_dependant_parser) {
    if (p_dependant_parser == nullptr) {
        return nullptr;
    }

    String script_path = p_class->get_datatype().script_path;
    if (HashMap<String, Ref<OScriptParserRef>>::Iterator E = p_dependant_parser->depended_parsers.find(script_path)) {
        if (E->value.is_valid()) {
            // Silently ensure it's parsed
            E->value->raise_status(OScriptParserRef::PARSED);
            if (E->value->get_parser()->has_class(p_class)) {
...
Ref<OScriptParserRef> OScriptAnalyzer::ensure_cached_external_parser_for_class(const OScriptParser::ClassNode* p_class, const OScriptParser::ClassNode* p_from_class, const char* p_context, const OScriptParser::Node* p_source) {
    // Delicate piece of code that intentionally doesn't use the OScript cache or `get_depended_parser_for`.
    ...
    if (HashMap<const OScriptParser::ClassNode*, Ref<OScriptParserRef>>::Iterator E = external_class_parser_cache.find(p_class)) {
        return E->value;
    }
    ...
    Ref<OScriptParserRef> parser_ref;
    for (const OScriptParser::ClassNode* look_class = p_from_class; look_class != nullptr; look_class = look_class->base_type.class_type) {
        ...
        if (HashMap<const OScriptParser::ClassNode*, Ref<OScriptParserRef>>::Iterator E = external_class_parser_cache.find(look_class)) {
        ...
        if (HashMap<String, Ref<OScriptParserRef>>::Iterator E = parser->depended_parsers.find(look_class_script_path)) {
            parser_ref = find_cached_external_parser_for_class(p_class, E->value);
            if (parser_ref.is_valid()) {
                break;
            }
...
OScriptParser::DataType OScriptAnalyzer::resolve_datatype(OScriptParser::TypeNode* p_type) {
    OScriptParser::DataType bad_type;
	bad_type.kind = OScriptParser::DataType::VARIANT;
	bad_type.type_source = OScriptParser::DataType::INFERRED;
	...
	OScriptParser::DataType resolving_datatype;
	resolving_datatype.kind = OScriptParser::DataType::RESOLVING;
	...
	const OScriptParser::IdentifierNode* first_id = p_type->type_chain[0];
	StringName first = first_id->name;
	bool type_found = false;

	if (first_id->suite && first_id->suite->has_local(first)) {
		const OScriptParser::SuiteNode::Local& local = first_id->suite->get_local(first);
		if (local.type == OScriptParser::SuiteNode::Local::CONSTANT) {
			result = local.get_data_type();
			...
			if (result.is_meta_type) {
				...
				Ref<OScript> oscript = local.constant->initializer->reduced_value;
				if (oscript.is_valid()) {
					Ref<OScriptParserRef> ref = parser->get_depended_parser_for(oscript->get_script_path());
					if (ref->raise_status(OScriptParserRef::INHERITANCE_SOLVED) != OK) {
						push_error(vformat(R"(Could not parse script from "%s".)", oscript->get_script_path()), first_id);
						return bad_type;
					}
	...
	if (!type_found) {
		if (first == StringName("Variant")) {
			if (p_type->type_chain.size() == 2) {
			    ...
			    if (GDE::CoreConstants::is_global_enum(qualified_name)) {
			...
			result.kind = OScriptParser::DataType::VARIANT;
		} else if (OScriptParser::get_builtin_type(first) < Variant::VARIANT_MAX) {
			// Built-in types.
			const Variant::Type builtin_type = OScriptParser::get_builtin_type(first);

			if (p_type->type_chain.size() == 2) {
				// May be nested enum.
				const StringName enum_name = p_type->type_chain[1]->name;
				if (GDE::Variant::has_enum(builtin_type, enum_name)) {
			...
			if (builtin_type == Variant::ARRAY) {
				OScriptParser::DataType container_type = type_from_metatype(resolve_datatype(p_type->get_container_type_or_null(0)));
				if (container_type.kind != OScriptParser::DataType::VARIANT) {
			...
			if (builtin_type == Variant::DICTIONARY) {
				OScriptParser::DataType key_type = type_from_metatype(resolve_datatype(p_type->get_container_type_or_null(0)));
				if (key_type.kind != OScriptParser::DataType::VARIANT) {
					key_type.is_constant = false;
					result.set_container_element_type(0, key_type);
				}
				OScriptParser::DataType value_type = type_from_metatype(resolve_datatype(p_type->get_container_type_or_null(1)));
				if (value_type.kind != OScriptParser::DataType::VARIANT) {
			...
			if (OScript::is_canonically_equal_paths(parser->script_path, ScriptServer::get_global_class_path(first))) {
				result = parser->head->get_datatype();
			} else {
				String path = ScriptServer::get_global_class_path(first);
				String ext = path.get_extension();
				if (ext == OScriptLanguage::get_singleton()->_get_extension()) {
					Ref<OScriptParserRef> ref = parser->get_depended_parser_for(path);
					if (ref.is_null() || ref->raise_status(OScriptParserRef::INHERITANCE_SOLVED) != OK) {
			...
			if (is_packed_scene(autoload_path)) {
				...
				if (OScriptLanguage::get_singleton()->has_any_global_constant(first)) {
					...
					if (node != nullptr) {
					    Ref<OScript> scr = node->get_script();
						if (scr.is_valid()) {
		...
		} else if (GDE::CoreConstants::is_global_enum(first)) {
			...
			List<OScriptParser::ClassNode*> script_classes;
			bool found = false;
			get_class_node_current_scope_classes(parser->current_class, &script_classes, p_type);

			for (OScriptParser::ClassNode* script_class : script_classes) {
				if (found) {
					break;
				}
				...
				if (script_class->members_indices.has(first)) {
					...
					OScriptParser::ClassNode::Member member = script_class->get_member(first);
					switch (member.type) {
						case OScriptParser::ClassNode::Member::CLASS: {
						    result = member.get_data_type();
						    found = true;
						    break;
						}
						...
						case OScriptParser::ClassNode::Member::CONSTANT: {
						    if (member.get_data_type().is_meta_type) {
						        result = member.get_data_type();
						        found = true;
						        break;
						    } else if (Ref<Script>(member.constant->initializer->reduced_value).is_valid()) {
						        Ref<OScript> gdscript = member.constant->initializer->reduced_value;
						        if (gdscript.is_valid()) {
						            Ref<OScriptParserRef> ref = parser->get_depended_parser_for(gdscript->get_script_path());
						            if (ref->raise_status(OScriptParserRef::INHERITANCE_SOLVED) != OK) {
	...
	if (p_type->type_chain.size() > 1) {
		if (result.kind == OScriptParser::DataType::CLASS) {
			for (int i = 1; i < p_type->type_chain.size(); i++) {
				OScriptParser::DataType base = result;
		...
		} else if (result.kind == OScriptParser::DataType::NATIVE) {
...
OScriptParser::DataType OScriptAnalyzer::type_from_variant(const Variant& p_value, const OScriptParser::Node* p_source) {
	...
	if (p_value.get_type() == Variant::ARRAY) {
		...
		if (array.get_typed_script()) {
			...
			result.set_container_element_type(0, type_from_metatype(make_builtin_meta_type(GDE::Variant::as_type(array.get_typed_builtin()))));
		}
		...
		if (dict.get_typed_value_script()) {
			...
			result.set_container_element_type(1, type_from_metatype(make_builtin_meta_type(GDE::Variant::as_type(dict.get_typed_value_builtin()))));
		}
...
OScriptParser::DataType OScriptAnalyzer::type_from_property(const PropertyInfo& p_property, bool p_is_arg, bool p_is_readonly) const {
    OScriptParser::DataType result;
	...
	if (p_property.type == Variant::OBJECT) {
		...
		if (p_property.type == Variant::ARRAY && p_property.hint == PROPERTY_HINT_ARRAY_TYPE) {
			...
			OScriptParser::DataType elem_type;
			elem_type.type_source = OScriptParser::DataType::ANNOTATED_EXPLICIT;

			Variant::Type elem_builtin_type = OScriptParser::get_builtin_type(elem_type_name);
			if (elem_builtin_type < Variant::VARIANT_MAX) {
				// Builtin type.
				elem_type.kind = OScriptParser::DataType::BUILTIN;
				elem_type.builtin_type = elem_builtin_type;
			} else if (class_exists(elem_type_name)) {
				elem_type.kind = OScriptParser::DataType::NATIVE;
				...
				elem_type.kind = OScriptParser::DataType::SCRIPT;
				elem_type.builtin_type = Variant::OBJECT;
			...
			OScriptParser::DataType key_elem_type;
			key_elem_type.type_source = OScriptParser::DataType::ANNOTATED_EXPLICIT;

			Variant::Type key_elem_builtin_type = OScriptParser::get_builtin_type(key_elem_type_name);
			if (key_elem_builtin_type < Variant::VARIANT_MAX) {
				// Builtin type.
				key_elem_type.kind = OScriptParser::DataType::BUILTIN;
				key_elem_type.builtin_type = key_elem_builtin_type;
			} else if (class_exists(key_elem_type_name)) {
				key_elem_type.kind = OScriptParser::DataType::NATIVE;
				...
				key_elem_type.kind = OScriptParser::DataType::SCRIPT;
				key_elem_type.builtin_type = Variant::OBJECT;
			...
			OScriptParser::DataType value_elem_type;
			value_elem_type.type_source = OScriptParser::DataType::ANNOTATED_EXPLICIT;

			Variant::Type value_elem_builtin_type = OScriptParser::get_builtin_type(value_elem_type_name);
			if (value_elem_builtin_type < Variant::VARIANT_MAX) {
				// Builtin type.
				value_elem_type.kind = OScriptParser::DataType::BUILTIN;
				value_elem_type.builtin_type = value_elem_builtin_type;
			} else if (class_exists(value_elem_type_name)) {
				value_elem_type.kind = OScriptParser::DataType::NATIVE;
				...
				value_elem_type.kind = OScriptParser::DataType::SCRIPT;
				value_elem_type.builtin_type = Variant::OBJECT;
			...
			if ((p_property.usage & PROPERTY_USAGE_CLASS_IS_ENUM) && p_property.class_name != StringName()) {
				if (GDE::CoreConstants::is_global_enum(p_property.class_name)) {
					result = make_global_enum_type(p_property.class_name, StringName(), false);
					result.is_constant = false;
				} else {
					const PackedStringArray names = p_property.class_name.split(ENUM_SEPARATOR);
					if (names.size() == 2) {
...
OScriptParser::DataType OScriptAnalyzer::make_global_class_meta_type(const StringName& p_class_name, const OScriptParser::Node* p_source) {
    OScriptParser::DataType type;
    ...
    if (ext == OScriptLanguage::get_singleton()->_get_extension()) {
        ...
        if (ref.is_null()) {
            ...
            type.type_source = OScriptParser::DataType::UNDETECTED;
            type.kind = OScriptParser::DataType::VARIANT;
            return type;
        }
...
OScriptParser::DataType OScriptAnalyzer::get_operation_type(Variant::Operator p_operation, const OScriptParser::DataType& p_a, const OScriptParser::DataType& p_b, bool& r_valid, const OScriptParser::Node* p_source) {
    if (p_operation == Variant::OP_AND || p_operation == Variant::OP_OR) {
        ...
        r_valid = true;
        OScriptParser::DataType result;
        result.type_source = OScriptParser::DataType::ANNOTATED_INFERRED;
        result.kind = OScriptParser::DataType::BUILTIN;
    ...
    if (p_a.kind == OScriptParser::DataType::ENUM) {
        if (p_a.is_meta_type) {
    ...
    if (p_b.kind == OScriptParser::DataType::ENUM) {
        if (p_b.is_meta_type) {
    ...
    if (p_operation == Variant::OP_ADD && a_type == Variant::ARRAY && b_type == Variant::ARRAY) {
        if (p_a.has_container_element_type(0) && p_b.has_container_element_type(0) && p_a.get_container_element_type(0) == p_b.get_container_element_type(0)) {
            r_valid = true;
            result = p_a;
            result.type_source = hard_operation ? OScriptParser::DataType::ANNOTATED_INFERRED : OScriptParser::DataType::INFERRED;
            return result;
        }
    }

    GDExtensionPtrOperatorEvaluator op_eval = GDE::Variant::get_validated_operator_evaluator(p_operation, a_type, b_type);
    ...
    if (validated) {
        ...
        result.type_source = hard_operation ? OScriptParser::DataType::ANNOTATED_INFERRED : OScriptParser::DataType::INFERRED;
        result.kind = OScriptParser::DataType::BUILTIN;
        result.builtin_type = GDE::Variant::get_operator_return_type(p_operation, a_type, b_type);
    } else {
...
OScriptParser::DataType OScriptAnalyzer::get_operation_type(Variant::Operator p_operation, const OScriptParser::DataType& p_a, bool& r_valid, const OScriptParser::Node* p_source) {
    // Unary version.
    OScriptParser::DataType nil_type;
    nil_type.builtin_type = Variant::NIL;
    nil_type.type_source = OScriptParser::DataType::ANNOTATED_INFERRED;
    return get_operation_type(p_operation, p_a, nil_type, r_valid, p_source);
}

bool OScriptAnalyzer::has_member_name_conflict_in_native_type(const StringName& p_name, const StringName& p_native_type) {
    if (ClassDB::class_has_signal(p_native_type, p_name)) {
        return true;
    }
    if (GDE::ClassDB::has_property(p_native_type, p_name)) {
        return true;
    }
    if (ClassDB::class_has_integer_constant(p_native_type, p_name)) {
        return true;
    }
...
bool OScriptAnalyzer::has_member_name_conflict_in_script_class(const StringName& p_name, const OScriptParser::ClassNode* p_class, const OScriptParser::Node* p_member) {
    if (p_class->members_indices.has(p_name)) {
        int index = p_class->members_indices[p_name];

        const OScriptParser::ClassNode::Member* member = &p_class->members[index];
        if (member->type == OScriptParser::ClassNode::Member::VARIABLE ||
                member->type == OScriptParser::ClassNode::Member::CONSTANT ||
                member->type == OScriptParser::ClassNode::Member::ENUM ||
                member->type == OScriptParser::ClassNode::Member::ENUM_VALUE ||
                member->type == OScriptParser::ClassNode::Member::CLASS ||
                member->type == OScriptParser::ClassNode::Member::SIGNAL) {
            return true;
        }

        if (p_member->type != OScriptParser::Node::FUNCTION && member->type == OScriptParser::ClassNode::Member::FUNCTION) {
            return true;
        }
    }
...
bool OScriptAnalyzer::get_function_signature(OScriptParser::Node* p_source, bool p_is_constructor, OScriptParser::DataType p_base_type, const StringName& p_function, OScriptParser::DataType& r_return_type, List<OScriptParser::DataType>& r_par_types, int& r_default_arg_count, BitField<MethodFlags>& r_method_flags, StringName* r_native_class) {
	...
	bool was_enum = false;
	if (p_base_type.kind == OScriptParser::DataType::ENUM) {
		was_enum = true;
		if (p_base_type.is_meta_type) {
			// Enum type can be treated as a dictionary value.
			p_base_type.kind = OScriptParser::DataType::BUILTIN;
	...
	if (p_base_type.kind == OScriptParser::DataType::BUILTIN) {
		// Construct a base type to get methods.
	    Variant dummy;
	    GDExtensionCallError err = GDE::Variant::construct(p_base_type.builtin_type, dummy, nullptr, 0);
		if (err.error != GDEXTENSION_CALL_OK) {
			ERR_FAIL_V_MSG(false, "Could not construct base Variant type.");
		}

	    const Vector<MethodInfo> methods = GDE::Variant::get_method_list(dummy);
	...
	if (base_native != StringName()) {
		...
		if (!class_exists(base_native)) {
			...
			return false;
		} else if (p_is_constructor && GDE::ClassDB::is_abstract(base_native)) {
			if (p_base_type.kind == OScriptParser::DataType::CLASS) {
				push_error(vformat(R"(Class "%s" cannot be constructed as it is based on abstract native class "%s".)",
				    p_base_type.class_type->fqcn.get_file(), base_native), p_source);
			} else if (p_base_type.kind == OScriptParser::DataType::SCRIPT) {
				push_error(vformat(R"(Script "%s" cannot be constructed as it is based on abstract native class "%s".)",
				    p_base_type.script_path.get_file(), base_native), p_source);
			} else {
	...
	if (p_is_constructor) {
		function_name = OScriptLanguage::get_singleton()->strings._init;
		r_method_flags.set_flag(METHOD_FLAG_STATIC);
	}

	OScriptParser::ClassNode* base_class = p_base_type.class_type;
	OScriptParser::FunctionNode* found_function = nullptr;

	while (found_function == nullptr && base_class != nullptr) {
		if (base_class->has_member(function_name)) {
			if (base_class->get_member(function_name).type != OScriptParser::ClassNode::Member::FUNCTION) {
	...
	if (p_base_type.is_meta_type && !p_is_constructor && (p_base_type.kind == OScriptParser::DataType::SCRIPT || p_base_type.kind == OScriptParser::DataType::CLASS)) {
		MethodInfo info;
		StringName script_class = p_base_type.kind == OScriptParser::DataType::SCRIPT
	        ? StringName(p_base_type.script_type->get_class())
	        : StringName(OScript::get_class_static());

		if (GDE::ClassDB::get_method_info(script_class, function_name, info)) {
			return function_signature_from_info(info, r_return_type, r_par_types, r_default_arg_count, r_method_flags);
		}
	}
	...
	if (p_is_constructor) {
		...
		r_return_type.type_source = OScriptParser::DataType::ANNOTATED_EXPLICIT;
		r_return_type.is_meta_type = false;
		return true;
	}

	MethodInfo info;
	if (GDE::ClassDB::get_method_info(base_native, function_name, info)) {
        ...
        #ifdef DEBUG_ENABLED
	    // In GDScriptAnalyzer, this used ClassDB::get_method to get the MethodBind; however, this does not
...
bool OScriptAnalyzer::function_signature_from_info(const MethodInfo& p_info, OScriptParser::DataType& r_return_type, List<OScriptParser::DataType>& r_par_types, int& r_default_arg_count, BitField<MethodFlags>& r_method_flags) {
...
Error OScriptAnalyzer::check_native_member_name_conflict(const StringName& p_member_name, const OScriptParser::Node* p_member, const StringName& p_native_type) {
    if (has_member_name_conflict_in_native_type(p_member_name, p_native_type)) {
        push_error(vformat(R"(Member "%s" redefined (original in native class '%s'))", p_member_name, p_native_type), p_member);
        return ERR_PARSE_ERROR;
    }

    if (class_exists(p_member_name)) {
        push_error(vformat(R"(The member "%s" shadows a native class.)", p_member_name), p_member);
        return ERR_PARSE_ERROR;
    }

    if (OScriptParser::get_builtin_type(p_member_name) < Variant::VARIANT_MAX) {
        push_error(vformat(R"(The member "%s" cannot have the same name as a builtin type.)", p_member_name), p_member);
        return ERR_PARSE_ERROR;
    }
...
Error OScriptAnalyzer::check_class_member_name_conflict(const OScriptParser::ClassNode* p_class, const StringName& p_member_name, const OScriptParser::Node* p_member) {
    // TODO check outer classes for static members only
    const OScriptParser::DataType* current_data_type = &p_class->base_type;
    while (current_data_type && current_data_type->kind == OScriptParser::DataType::Kind::CLASS) {
        OScriptParser::ClassNode* current_class_node = current_data_type->class_type;
        if (has_member_name_conflict_in_script_class(p_member_name, current_class_node, p_member)) {
            ...
            push_error(vformat(R"(The member "%s" already exists in parent class %s.)", p_member_name, parent_class_name), p_member);
            return ERR_PARSE_ERROR;
        }
    ...
    if (current_data_type && current_data_type->kind == OScriptParser::DataType::Kind::NATIVE) {
...
Error OScriptAnalyzer::resolve_class_inheritance(OScriptParser::ClassNode* p_class, const OScriptParser::Node* p_source) { // NOLINT
    if (p_source == nullptr && parser->has_class(p_class)) {
        p_source = p_class;
    }

    Ref<OScriptParserRef> parser_ref = ensure_cached_external_parser_for_class(p_class, nullptr, "Trying to resolve class inheritance", p_source);
    Finally finally([&]() {
        for (OScriptParser::ClassNode* look_class = p_class; look_class != nullptr; look_class = look_class->base_type.class_type) {
    ...
    if (!parser->has_class(p_class)) {
        ...
        Error err = parser_ref->raise_status(OScriptParserRef::PARSED);
        if (err) {
            push_error(vformat(R"(Could not parse script "%s": %s.)", p_class->get_datatype().script_path, error_names[err]), p_source);
            return ERR_PARSE_ERROR;
        }

        OScriptAnalyzer* other_analyzer = parser_ref->get_analyzer();
        OScriptParser* other_parser = parser_ref->get_parser();
    ...
    OScriptParser::ClassNode* previous_class = parser->current_class;
    parser->current_class = p_class;

    if (p_class->identifier) {
        StringName class_name = p_class->identifier->name;
        if (OScriptParser::get_builtin_type(class_name) < Variant::VARIANT_MAX) {
    ...
    OScriptParser::DataType class_type;
    class_type.is_constant = true;
    class_type.is_meta_type = true;
    class_type.type_source = OScriptParser::DataType::ANNOTATED_EXPLICIT;
    class_type.kind = OScriptParser::DataType::CLASS;
    ...
    if (!p_class->extends_used) {
        ...
        result.kind = OScriptParser::DataType::NATIVE;
        result.builtin_type = Variant::OBJECT;
        result.native_type = StringName("RefCounted");
    } else {
        result.type_source = OScriptParser::DataType::ANNOTATED_EXPLICIT;
        OScriptParser::DataType base;
        int extends_index = 0;
        ...
        if (!p_class->extends_path.is_empty()) {
			...
			Ref<OScriptParserRef> ext_parser = parser->get_depended_parser_for(p_class->extends_path);
			if (ext_parser.is_null()) {
				push_error(vformat(R"(Could not resolve super class path "%s".)", p_class->extends_path), p_class);
				return ERR_PARSE_ERROR;
			}

			Error err = ext_parser->raise_status(OScriptParserRef::INHERITANCE_SOLVED);
			if (err != OK) {
			...
			if (!parser->_is_tool && ext_parser->get_parser()->_is_tool) {
				parser->push_warning(p_class, OScriptWarning::MISSING_TOOL);
			}
            #endif // DEBUG_ENABLED
			...
			OScriptParser::IdentifierNode *id = p_class->extends[extends_index++];
			const StringName &name = id->name;
			base.type_source = OScriptParser::DataType::ANNOTATED_EXPLICIT;

			if (ScriptServer::is_global_class(name)) {
				String base_path = ScriptServer::get_global_class_path(name);
				if (OScript::is_canonically_equal_paths(base_path, parser->script_path)) {
					base = parser->head->get_datatype();
				} else {
					Ref<OScriptParserRef> base_parser = parser->get_depended_parser_for(base_path);
					if (base_parser.is_null()) {
						push_error(vformat(R"(Could not resolve super class "%s".)", name), id);
						return ERR_PARSE_ERROR;
					}

					Error err = base_parser->raise_status(OScriptParserRef::INHERITANCE_SOLVED);
					if (err != OK) {
				...
				Ref<OScriptParserRef> info_parser = parser->get_depended_parser_for(script_path);
				if (info_parser.is_null()) {
					push_error(vformat(R"(Could not parse singleton from "%s".)", script_path), id);
					return ERR_PARSE_ERROR;
				}

				Error err = info_parser->raise_status(OScriptParserRef::INHERITANCE_SOLVED);
				if (err != OK) {
					push_error(vformat(R"(Could not resolve super class inheritance from "%s".)", name), id);
					return err;
				}
				...
				base.kind = OScriptParser::DataType::NATIVE;
				...
				for (OScriptParser::ClassNode *look_class : script_classes) {
					...
					if (look_class->has_member(name)) {
						...
						OScriptParser::ClassNode::Member member = look_class->get_member(name);
						OScriptParser::DataType member_datatype = member.get_data_type();
						...
						switch (member.type) {
							...
							case OScriptParser::ClassNode::Member::CONSTANT: {
							    if (member_datatype.kind != OScriptParser::DataType::SCRIPT && member_datatype.kind != OScriptParser::DataType::CLASS) {
		...
		for (int index = extends_index; index < p_class->extends.size(); index++) {
			OScriptParser::IdentifierNode *id = p_class->extends[index];

			if (base.kind != OScriptParser::DataType::CLASS) {
			...
			OScriptParser::DataType id_type = id->get_datatype();

			if (!id_type.is_set()) {
				push_error(vformat(R"(Could not find nested type "%s".)", id->name), id);
				return ERR_PARSE_ERROR;
			} else if (id_type.kind != OScriptParser::DataType::SCRIPT && id_type.kind != OScriptParser::DataType::CLASS) {
    ...
    const OScriptParser::ClassNode *base_class = result.class_type;
    while (base_class) {
    ...
    // Apply annotations.
    for (OScriptParser::AnnotationNode *&E : p_class->annotations) {
...
Error OScriptAnalyzer::resolve_class_inheritance(OScriptParser::ClassNode* p_class, bool p_recursive) {
...
void OScriptAnalyzer::resolve_annotation(OScriptParser::AnnotationNode* p_node) {
    ERR_FAIL_COND_MSG(!parser->valid_annotations.has(p_node->name), vformat(R"(Annotation "%s" not found to validate.)", p_node->name));

    if (p_node->is_resolved) {
        return;
    }
    ...
    for (uint64_t i = 0, j = 0; i < node_arguments_size; i++) {
        ...
        if (value.get_type() != argument_info.type) {
            ...
            if (argument_info.type == Variant::INT && value.get_type() == Variant::FLOAT) {
                parser->push_warning(argument, OScriptWarning::NARROWING_CONVERSION);
            }
            #endif // DEBUG_ENABLED
            ...
            if (!Variant::can_convert_strict(value.get_type(), argument_info.type)) {
                push_error(vformat(R"(Invalid argument for annotation "%s": argument %d should be "%s" but is "%s".)",
                    p_node->name, i + 1, Variant::get_type_name(argument_info.type), argument->get_datatype().to_string()), argument);
                return;
            }
            ...
            GDExtensionCallError ce = GDE::Variant::construct(argument_info.type, converted_to, &converted_from, 1);
            if (ce.error != GDEXTENSION_CALL_OK) {
                push_error(vformat(R"(Cannot convert argument %d of annotation "%s" from "%s" to "%s".)", i + 1,
                    p_node->name, Variant::get_type_name(value.get_type()), Variant::get_type_name(argument_info.type)), argument);
                return;
            }
        ...
        p_node->resolved_arguments.push_back(value);
    }
}

void OScriptAnalyzer::resolve_class_member(OScriptParser::ClassNode* p_class, const StringName& p_name, const OScriptParser::Node* p_source) {
    ERR_FAIL_COND(!p_class->has_member(p_name));
    resolve_class_member(p_class, p_class->members_indices[p_name], p_source);
}

void OScriptAnalyzer::resolve_class_member(OScriptParser::ClassNode* p_class, int p_index, const OScriptParser::Node* p_source) {
    ERR_FAIL_INDEX(p_index, p_class->members.size());

    OScriptParser::ClassNode::Member& member = p_class->members.write[p_index];
    if (p_source == nullptr && parser->has_class(p_class)) {
        p_source = member.get_source_node();
    }

    Ref<OScriptParserRef> parser_ref = ensure_cached_external_parser_for_class(p_class, nullptr, "Trying to resolve class member", p_source);
    Finally finally([&]() {
        ensure_cached_external_parser_for_class(member.get_data_type().class_type, p_class, "Trying to resolve datatype of class member", p_source);
        OScriptParser::DataType member_type = member.get_data_type();
    ...
    if (!parser->has_class(p_class)) {
        ...
        Error err = parser_ref->raise_status(OScriptParserRef::PARSED);
        if (err) {
            push_error(vformat(R"(Could not parse script "%s": %s (While resolving external class member "%s").)",
                p_class->get_datatype().script_path, error_names[err], member.get_name()), p_source);
            return;
        }
    ...
    {
        #ifdef DEBUG_ENABLED
		OScriptParser::Node* member_node = member.get_source_node();
		if (member_node && member_node->type != OScriptParser::Node::ANNOTATION) {
			// Apply @warning_ignore annotations before resolving member.
			for (OScriptParser::AnnotationNode*& E : member_node->annotations) {
		...
		switch (member.type) {
			case OScriptParser::ClassNode::Member::VARIABLE: {
				...
				// Apply annotations.
				for (OScriptParser::AnnotationNode*& E : member.variable->annotations) {
					if (E->name != StringName("@warning_ignore")) {
						resolve_annotation(E);
						E->apply(parser, member.variable, p_class);
					}
				...
				if (member.variable->exported && member.variable->onready) {
					parser->push_warning(member.variable, OScriptWarning::ONREADY_WITH_EXPORT);
				}
				...
				if (member.variable->initializer) {
					...
					if (!member.variable->is_static &&
					    !member.variable->onready &&
					    member.variable->initializer && (
					        member.variable->initializer->type == OScriptParser::Node::GET_NODE ||
					        member.variable->initializer->type == OScriptParser::Node::CALL ||
					        member.variable->initializer->type == OScriptParser::Node::CAST)) {
						OScriptParser::Node* expr = member.variable->initializer;
						if (expr->type == OScriptParser::Node::CAST) {
							expr = static_cast<OScriptParser::CastNode *>(expr)->operand;
						}

						bool is_get_node = expr->type == OScriptParser::Node::GET_NODE;
						bool is_using_shorthand = is_get_node;

						if (!is_get_node && expr->type == OScriptParser::Node::CALL) {
							is_using_shorthand = false;
							OScriptParser::CallNode* call = static_cast<OScriptParser::CallNode*>(expr);
							if (call->function_name == StringName("get_node")) {
								switch (call->get_callee_type()) {
									...
									case OScriptParser::Node::SUBSCRIPT: {
										OScriptParser::SubscriptNode* subscript = static_cast<OScriptParser::SubscriptNode*>(call->callee);
										is_get_node = subscript->is_attribute && subscript->base->type == OScriptParser::Node::SELF;
									} break;
						...
						if (is_get_node) {
							...
							if (is_using_shorthand) {
								OScriptParser::GetNodeNode* get_node_node = static_cast<OScriptParser::GetNodeNode*>(expr);
								offending_syntax = get_node_node->use_dollar ? "$" : "%";
							}
							parser->push_warning(member.variable, OScriptWarning::GET_NODE_DEFAULT_WITHOUT_ONREADY, offending_syntax);
						}
					}
				}
                #endif // DEBUG_ENABLED
			...
			case OScriptParser::ClassNode::Member::CONSTANT: {
				...
				// Apply annotations.
				for (OScriptParser::AnnotationNode*& E : member.constant->annotations) {
					resolve_annotation(E);
			...
			case OScriptParser::ClassNode::Member::SIGNAL: {
				...
				for (int j = 0; j < member.signal->parameters.size(); j++) {
					OScriptParser::ParameterNode* param = member.signal->parameters[j];
					OScriptParser::DataType param_type = type_from_metatype(resolve_datatype(param->datatype_specifier));
					param->set_datatype(param_type);

                    #ifdef DEBUG_ENABLED
					if (param->datatype_specifier == nullptr) {
						parser->push_warning(param, OScriptWarning::UNTYPED_DECLARATION, "Parameter", param->identifier->name);
					}
                    #endif // DEBUG_ENABLED
				...
				member.signal->method = mi;

				// Apply annotations.
				for (OScriptParser::AnnotationNode *&E : member.signal->annotations) {
					resolve_annotation(E);
			...
			case OScriptParser::ClassNode::Member::ENUM: {
				...
				OScriptParser::DataType enum_type = make_class_enum_type(member.m_enum->identifier->name, p_class, parser->script_path, true);

				const OScriptParser::EnumNode* prev_enum = current_enum;
				current_enum = member.m_enum;

				Dictionary dictionary;
				for (int j = 0; j < member.m_enum->values.size(); j++) {
					OScriptParser::EnumNode::Value& element = member.m_enum->values.write[j];

					if (element.expression) {
						reduce_expression(element.expression);
						if (!element.expression->is_constant) {
							push_error(R"(Enum values must be constant.)", element.expression);
						} else if (element.expression->reduced_value.get_type() != Variant::INT) {
							push_error(R"(Enum values must be integers.)", element.expression);
						} else {
							element.value = element.expression->reduced_value;
				...
				// Apply annotations.
				for (OScriptParser::AnnotationNode*& E : member.m_enum->annotations) {
					resolve_annotation(E);
			...
			case OScriptParser::ClassNode::Member::FUNCTION: {
			    for (OScriptParser::AnnotationNode*& E : member.function->annotations) {
			        resolve_annotation(E);
			...
			case OScriptParser::ClassNode::Member::ENUM_VALUE: {
				...
				if (member.enum_value.expression) {
					check_class_member_name_conflict(p_class, member.enum_value.identifier->name, member.enum_value.expression);

					const OScriptParser::EnumNode* prev_enum = current_enum;
					current_enum = member.enum_value.parent_enum;
					reduce_expression(member.enum_value.expression);
					current_enum = prev_enum;

					if (!member.enum_value.expression->is_constant) {
						push_error(R"(Enum values must be constant.)", member.enum_value.expression);
					} else if (member.enum_value.expression->reduced_value.get_type() != Variant::INT) {
						push_error(R"(Enum values must be integers.)", member.enum_value.expression);
					} else {
						member.enum_value.value = member.enum_value.expression->reduced_value;
					...
					if (member.enum_value.index > 0) {
						const OScriptParser::EnumNode::Value& prev_value = member.enum_value.parent_enum->values[member.enum_value.index - 1];
			...
			case OScriptParser::ClassNode::Member::UNDEFINED: {
...
void OScriptAnalyzer::resolve_class_interface(OScriptParser::ClassNode* p_class, const OScriptParser::Node* p_source) {
    if (p_source == nullptr && parser->has_class(p_class)) {
		p_source = p_class;
	}

	Ref<OScriptParserRef> parser_ref = ensure_cached_external_parser_for_class(p_class, nullptr, "Trying to resolve class interface", p_source);
	...
	if (!p_class->resolved_interface) {
		...
		OScriptParser::DataType base_type = p_class->base_type;
		if (base_type.kind == OScriptParser::DataType::CLASS) {
			OScriptParser::ClassNode* base_class = base_type.class_type;
		...
		for (int i = 0; i < p_class->members.size(); i++) {
            ...
            #ifdef DEBUG_ENABLED
			if (!has_static_data) {
				OScriptParser::ClassNode::Member member = p_class->members[i];
		...
		if (!has_static_data && p_class->annotated_static_unload) {
			OScriptParser::Node* static_unload = nullptr;
			for (OScriptParser::AnnotationNode* node : p_class->annotations) {
				if (node->name == StringName("@static_unload")) {
					static_unload = node;
					break;
				}
			}
			parser->push_warning(static_unload ? static_unload : p_class, OScriptWarning::REDUNDANT_STATIC_UNLOAD);
		}
        #endif // DEBUG_ENABLED
	}
}

void OScriptAnalyzer::resolve_class_interface(OScriptParser::ClassNode* p_class, bool p_recursive) {
...
void OScriptAnalyzer::resolve_class_body(OScriptParser::ClassNode* p_class, const OScriptParser::Node* p_source) {
    if (p_source == nullptr && parser->has_class(p_class)) {
		p_source = p_class;
	}

	Ref<OScriptParserRef> parser_ref = ensure_cached_external_parser_for_class(p_class, nullptr, "Trying to resolve class body", p_source);
	...
	for (int i = 0; i < p_class->members.size(); i++) {
		...
		} else if (member.type == OScriptParser::ClassNode::Member::VARIABLE && member.variable->style != OScriptParser::VariableNode::NONE) {
			if (member.variable->style == OScriptParser::VariableNode::INLINE) {
				if (member.variable->getter != nullptr) {
					member.variable->getter->return_type = member.variable->datatype_specifier;
					member.variable->getter->set_datatype(member.get_data_type());
				...
				if (member.variable->setter != nullptr) {
					...
					member.variable->setter->parameters[0]->set_datatype(member.get_data_type());
					resolve_function_body(member.variable->setter);
				}
			}
		} else if (member.type == OScriptParser::ClassNode::Member::GROUP) {
	...
	for (int i = 0; i < p_class->members.size(); i++) {
		...
		if (member.type == OScriptParser::ClassNode::Member::VARIABLE) {
			...
			if (member.variable->usages == 0 && String(member.variable->identifier->name).begins_with("_")) {
				parser->push_warning(member.variable->identifier, OScriptWarning::UNUSED_PRIVATE_CLASS_VARIABLE, member.variable->identifier->name);
			}
            #endif // DEBUG_ENABLED

			if (member.variable->style == OScriptParser::VariableNode::SETGET) {
				OScriptParser::FunctionNode* getter_function = nullptr;
				OScriptParser::FunctionNode* setter_function = nullptr;
				...
				if (member.variable->getter_pointer != nullptr) {
					...
					if (getter_function == nullptr) {
						...
						OScriptParser::DataType return_datatype = getter_function->data_type;
						if (getter_function->return_type != nullptr) {
							return_datatype = getter_function->return_type->data_type;
						...
						} else if (!is_type_compatible(member.variable->data_type, return_datatype, true)) {
							push_error(vformat(R"(Function with return type "%s" cannot be used as getter for a property of type "%s".)",
							    return_datatype.to_string(), member.variable->data_type.to_string()), member.variable);
						} else {
							has_valid_getter = true;
                            #ifdef DEBUG_ENABLED
							if (member.variable->data_type.builtin_type == Variant::INT && return_datatype.builtin_type == Variant::FLOAT) {
								parser->push_warning(member.variable, OScriptWarning::NARROWING_CONVERSION);
							}
                            #endif // DEBUG_ENABLED
						}
					}
				...
				if (member.variable->setter_pointer != nullptr) {
					...
					} else if (!is_type_compatible(member.variable->data_type, setter_function->parameters[0]->data_type, true)) {
						push_error(vformat(R"(Function with argument type "%s" cannot be used as setter for a property of type "%s".)",
						    setter_function->parameters[0]->data_type.to_string(), member.variable->data_type.to_string()), member.variable);
					} else {
						has_valid_setter = true;
                        #ifdef DEBUG_ENABLED
						if (member.variable->data_type.builtin_type == Variant::FLOAT && setter_function->parameters[0]->data_type.builtin_type == Variant::INT) {
							parser->push_warning(member.variable, OScriptWarning::NARROWING_CONVERSION);
						}
                        #endif // DEBUG_ENABLED
					}
				...
				if (member.variable->data_type.is_variant() && has_valid_getter && has_valid_setter) {
					if (!is_type_compatible(getter_function->data_type, setter_function->parameters[0]->data_type, true)) {
						push_error(vformat(R"(Getter with type "%s" cannot be used along with setter of type "%s".)",
						    getter_function->data_type.to_string(), setter_function->parameters[0]->data_type.to_string()), member.variable);
					}
				}
			}
		} else if (member.type == OScriptParser::ClassNode::Member::SIGNAL) {
            #ifdef DEBUG_ENABLED
			if (member.signal->usages == 0) {
				parser->push_warning(member.signal->identifier, OScriptWarning::UNUSED_SIGNAL, member.signal->identifier->name);
			}
            #endif // DEBUG_ENABLED
		}
	}
	...
	if (!pending_body_resolution_lambdas.is_empty()) {
		ERR_PRINT("OScript bug (please report): Not all pending lambda bodies were resolved in time.");
		resolve_pending_lambda_bodies();
	}
	...
	if (!p_class->is_abstract) {
		...
		const OScriptParser::ClassNode* base_class = p_class;
		while (base_class != nullptr) {
			if (!base_class->is_abstract && base_class != p_class) {
				break;
			}

			for (OScriptParser::ClassNode::Member member : base_class->members) {
				if (member.type == OScriptParser::ClassNode::Member::FUNCTION) {
		    ...
		    if (base_class->base_type.kind == OScriptParser::DataType::CLASS) {
				base_class = base_class->base_type.class_type;
			} else if (base_class->base_type.kind == OScriptParser::DataType::SCRIPT) {
				Ref<OScriptParserRef> base_parser_ref = parser->get_depended_parser_for(base_class->base_type.script_path);
...
void OScriptAnalyzer::resolve_class_body(OScriptParser::ClassNode* p_class, bool p_recursive) {
...
void OScriptAnalyzer::resolve_function_signature(OScriptParser::FunctionNode* p_function, const OScriptParser::Node* p_source, bool p_is_lambda) {
    if (p_source == nullptr) {
		p_source = p_function;
	}
	...
	OScriptParser::FunctionNode* previous_function = parser->current_function;
	...
	OScriptParser::DataType prev_datatype = p_function->get_datatype();
	...
	for (int i = 0; i < p_function->parameters.size(); i++) {
		...
		if (p_function->parameters[i]->usages == 0 && !String(p_function->parameters[i]->identifier->name).begins_with("_") && !p_function->is_abstract) {
			parser->push_warning(p_function->parameters[i]->identifier, OScriptWarning::UNUSED_PARAMETER, function_visible_name, p_function->parameters[i]->identifier->name);
		}
	...
	if (p_function->is_vararg()) {
		...
		if (p_function->rest_parameter->datatype_specifier != nullptr) {
			OScriptParser::DataType specified_type = p_function->rest_parameter->get_datatype();
			if (specified_type.kind != OScriptParser::DataType::BUILTIN || specified_type.builtin_type != Variant::ARRAY) {
			...
			OScriptParser::DataType inferred_type;
			inferred_type.type_source = OScriptParser::DataType::INFERRED;
			inferred_type.kind = OScriptParser::DataType::BUILTIN;
			inferred_type.builtin_type = Variant::ARRAY;
			p_function->rest_parameter->set_datatype(inferred_type);
            #ifdef DEBUG_ENABLED
			parser->push_warning(p_function->rest_parameter, OScriptWarning::UNTYPED_DECLARATION, "Parameter", p_function->rest_parameter->identifier->name);
            #endif
		}
		...
		if (p_function->rest_parameter->usages == 0 && !String(p_function->rest_parameter->identifier->name).begins_with("_") && !p_function->is_abstract) {
			parser->push_warning(p_function->rest_parameter->identifier, OScriptWarning::UNUSED_PARAMETER, function_visible_name, p_function->rest_parameter->identifier->name);
		}
		is_shadowing(p_function->rest_parameter->identifier, "function parameter", true);
        #endif // DEBUG_ENABLED
	}

	if (!p_is_lambda && function_name == OScriptLanguage::get_singleton()->strings._init) {
		// Constructor.
		OScriptParser::DataType return_type = parser->current_class->get_datatype();
		return_type.is_meta_type = false;
		p_function->set_datatype(return_type);
		if (p_function->return_type) {
			OScriptParser::DataType declared_return = resolve_datatype(p_function->return_type);
			if (declared_return.kind != OScriptParser::DataType::BUILTIN || declared_return.builtin_type != Variant::NIL) {
				push_error("Constructor cannot have an explicit return type.", p_function->return_type);
			}
		}
	} else if (!p_is_lambda && function_name == OScriptLanguage::get_singleton()->strings._static_init) {
		// Static constructor.
		OScriptParser::DataType return_type;
		return_type.kind = OScriptParser::DataType::BUILTIN;
		...
		if (p_function->return_type != nullptr) {
			...
			OScriptParser::DataType return_type;
			return_type.type_source = OScriptParser::DataType::INFERRED;
			return_type.kind = OScriptParser::DataType::VARIANT;
			p_function->set_datatype(return_type);
		}
		...
		OScriptParser::DataType base_type = parser->current_class->base_type;
		base_type.is_meta_type = false;
		OScriptParser::DataType parent_return_type;
		List<OScriptParser::DataType> parameters_types;
		int default_par_count = 0;
		BitField<MethodFlags> method_flags = { 0 };
		...
		if (!p_is_lambda && get_function_signature(p_function, false, base_type, function_name, parent_return_type, parameters_types, default_par_count, method_flags, &native_base)) {
			...
			if (p_function->return_type != nullptr) {
				...
				OScriptParser::DataType return_type = p_function->get_datatype();
				if (return_type.is_variant()) {
				...
				} else if (return_type.kind == OScriptParser::DataType::BUILTIN && return_type.builtin_type == Variant::NIL) {
					...
					if (parent_return_type.is_hard_type() && !(parent_return_type.kind == OScriptParser::DataType::BUILTIN && parent_return_type.builtin_type == Variant::NIL)) {
						valid = false;
					}
				} else {
			...
			if (valid) {
				int i = 0;
				for (const OScriptParser::DataType& parent_par_type : parameters_types) {
					if (i >= p_function->parameters.size()) {
						break;
					}
					const OScriptParser::DataType& current_par_type = p_function->parameters[i]->data_type;
					i++;
			...
			if (!valid) {
				...
				int j = 0;
				for (const OScriptParser::DataType& par_type : parameters_types) {
					if (j > 0) {
						parent_signature += ", ";
					}
			...
			if (native_base != StringName()) {
				parser->push_warning(p_function, OScriptWarning::NATIVE_METHOD_OVERRIDE, function_name, native_base);
			}
            #endif // DEBUG_ENABLED
		}
        #endif // TOOLS_ENABLED
    ...
    #ifdef DEBUG_ENABLED
	if (p_function->return_type == nullptr) {
		parser->push_warning(p_function, OScriptWarning::UNTYPED_DECLARATION, "Function", function_visible_name);
	}
    #endif // DEBUG_ENABLED
...
void OScriptAnalyzer::resolve_function_body(OScriptParser::FunctionNode* p_function, bool p_is_lambda) {
    ...
    } else if (p_function->get_datatype().is_hard_type() &&
        (p_function->get_datatype().kind != OScriptParser::DataType::BUILTIN || p_function->get_datatype().builtin_type != Variant::NIL)) {
        if (!p_function->body->has_return && (p_is_lambda || p_function->identifier->name != OScriptLanguage::get_singleton()->strings._init)) {
            push_error(vformat(R"(Not all code paths return a value in function "%s".)", p_function->identifier->name), p_function);
        }
    }
...
void OScriptAnalyzer::resolve_node(OScriptParser::Node* p_node, bool p_is_root) {
    ERR_FAIL_NULL_MSG(p_node, "Trying to resolve type of a null node.");

	switch (p_node->type) {
	    case OScriptParser::Node::NONE: {
	        break; // Unreachable.
	    }
	    case OScriptParser::Node::CLASS: {
	        // NOTE: Currently this route is never executed, `resolve_class_*()` is called directly.
	        if (OK == resolve_class_inheritance(static_cast<OScriptParser::ClassNode*>(p_node), true)) {
	            resolve_class_interface(static_cast<OScriptParser::ClassNode*>(p_node), true);
	            resolve_class_body(static_cast<OScriptParser::ClassNode*>(p_node), true);
	        }
	        break;
	    }
	    case OScriptParser::Node::CONSTANT: {
	        resolve_constant(static_cast<OScriptParser::ConstantNode*>(p_node), true);
	        break;
	    }
	    case OScriptParser::Node::FOR: {
	        resolve_for(static_cast<OScriptParser::ForNode*>(p_node));
	        break;
	    }
	    case OScriptParser::Node::IF: {
	        resolve_if(static_cast<OScriptParser::IfNode*>(p_node));
	        break;
	    }
	    case OScriptParser::Node::SUITE: {
	        resolve_suite(static_cast<OScriptParser::SuiteNode*>(p_node));
	        break;
	    }
	    case OScriptParser::Node::VARIABLE: {
	        resolve_variable(static_cast<OScriptParser::VariableNode*>(p_node), true);
	        break;
	    }
	    case OScriptParser::Node::WHILE: {
	        resolve_while(static_cast<OScriptParser::WhileNode*>(p_node));
	        break;
	    }
	    case OScriptParser::Node::ANNOTATION: {
	        resolve_annotation(static_cast<OScriptParser::AnnotationNode*>(p_node));
	        break;
	    }
	    case OScriptParser::Node::ASSERT: {
	        resolve_assert(static_cast<OScriptParser::AssertNode*>(p_node));
	        break;
	    }
	    case OScriptParser::Node::MATCH: {
	        resolve_match(static_cast<OScriptParser::MatchNode*>(p_node));
	        break;
	    }
	    case OScriptParser::Node::MATCH_BRANCH: {
	        resolve_match_branch(static_cast<OScriptParser::MatchBranchNode*>(p_node), nullptr);
	        break;
	    }
	    case OScriptParser::Node::PARAMETER: {
	        resolve_parameter(static_cast<OScriptParser::ParameterNode*>(p_node));
	        break;
	    }
	    case OScriptParser::Node::PATTERN: {
	        resolve_match_pattern(static_cast<OScriptParser::PatternNode*>(p_node), nullptr);
	        break;
	    }
	    case OScriptParser::Node::RETURN: {
	        resolve_return(static_cast<OScriptParser::ReturnNode*>(p_node));
	        break;
	    }
		case OScriptParser::Node::TYPE: {
		    resolve_datatype(static_cast<OScriptParser::TypeNode*>(p_node));
	        break;
		}
		...
		case OScriptParser::Node::UNARY_OPERATOR: {
		    reduce_expression(static_cast<OScriptParser::ExpressionNode*>(p_node), p_is_root);
	        break;
		}
...
void OScriptAnalyzer::resolve_suite(OScriptParser::SuiteNode* p_suite) {
    for (int i = 0; i < p_suite->statements.size(); i++) {
        OScriptParser::Node *stmt = p_suite->statements[i];
        // Apply annotations.
        for (OScriptParser::AnnotationNode*& E : stmt->annotations) {
            resolve_annotation(E);
...
void OScriptAnalyzer::resolve_assignable(OScriptParser::AssignableNode* p_assignable, const char* p_kind) {
    OScriptParser::DataType type;
	type.kind = OScriptParser::DataType::VARIANT;

	bool is_constant = p_assignable->type == OScriptParser::Node::CONSTANT;
	...
	if (p_assignable->identifier != nullptr && p_assignable->identifier->suite != nullptr && p_assignable->identifier->suite->parent_block != nullptr) {
		if (p_assignable->identifier->suite->parent_block->has_local(p_assignable->identifier->name)) {
			const OScriptParser::SuiteNode::Local& local = p_assignable->identifier->suite->parent_block->get_local(p_assignable->identifier->name);
			parser->push_warning(p_assignable->identifier, OScriptWarning::CONFUSABLE_LOCAL_DECLARATION, local.get_name(), p_assignable->identifier->name);
		}
	}
    #endif // DEBUG_ENABLED

	OScriptParser::DataType specified_type;
	...
	if (p_assignable->initializer != nullptr) {
		...
		if (p_assignable->initializer->type == OScriptParser::Node::ARRAY) {
			OScriptParser::ArrayNode* array = static_cast<OScriptParser::ArrayNode *>(p_assignable->initializer);
		...
		} else if (p_assignable->initializer->type == OScriptParser::Node::DICTIONARY) {
			OScriptParser::DictionaryNode* dictionary = static_cast<OScriptParser::DictionaryNode *>(p_assignable->initializer);
		...
		OScriptParser::DataType initializer_type = p_assignable->initializer->get_datatype();
		if (p_assignable->infer_datatype) {
			if (!initializer_type.is_set() || initializer_type.has_no_type() || !initializer_type.is_hard_type()) {
				push_error(vformat(R"(Cannot infer the type of "%s" %s because the value doesn't have a set type.)",
				    p_assignable->identifier->name, p_kind), p_assignable->initializer);
			} else if (initializer_type.kind == OScriptParser::DataType::BUILTIN && initializer_type.builtin_type == Variant::NIL && !is_constant) {
				push_error(vformat(R"(Cannot infer the type of "%s" %s because the value is "null".)",
				    p_assignable->identifier->name, p_kind), p_assignable->initializer);
			}

            #ifdef DEBUG_ENABLED
			if (initializer_type.is_hard_type() && initializer_type.is_variant()) {
				parser->push_warning(p_assignable, OScriptWarning::INFERENCE_ON_VARIANT, p_kind);
			}
            #endif // DEBUG_ENABLED
		} else {
		...
		if (!has_specified_type) {
			...
			if (!type.is_set() || (type.is_hard_type() && type.kind == OScriptParser::DataType::BUILTIN && type.builtin_type == Variant::NIL && !is_constant)) {
				type.kind = OScriptParser::DataType::VARIANT;
			}

			if (p_assignable->infer_datatype || is_constant) {
				type.type_source = OScriptParser::DataType::ANNOTATED_INFERRED;
			} else {
				type.type_source = OScriptParser::DataType::INFERRED;
			}
			...
			if (initializer_type.is_variant() || !initializer_type.is_hard_type()) {
				...
				if (!is_constant && is_type_compatible(initializer_type, specified_type)) {
					...
					push_error(vformat(R"(Cannot assign a value of type %s to %s "%s" with specified type %s.)",
					    initializer_type.to_string(), p_kind, p_assignable->identifier->name, specified_type.to_string()), p_assignable->initializer);
				}
			} else if ((specified_type.has_container_element_type(0) &&
			    !initializer_type.has_container_element_type(0)) ||
			    (specified_type.has_container_element_type(1) && !initializer_type.has_container_element_type(1))) {
				mark_node_unsafe(p_assignable->initializer);
            #ifdef DEBUG_ENABLED
			} else if (specified_type.builtin_type == Variant::INT && initializer_type.builtin_type == Variant::FLOAT) {
				parser->push_warning(p_assignable->initializer, OScriptWarning::NARROWING_CONVERSION);
            #endif // DEBUG_ENABLED
			}
		}
	}
    ...
    #ifdef DEBUG_ENABLED
	const bool is_parameter = p_assignable->type == OScriptParser::Node::PARAMETER;
	if (!has_specified_type) {
		...
		if (p_assignable->infer_datatype || is_constant) {
			...
			const bool is_type_import = is_constant && p_assignable->initializer != nullptr && p_assignable->initializer->data_type.is_meta_type;
			if (!is_type_import) {
				parser->push_warning(p_assignable, OScriptWarning::INFERRED_DECLARATION, declaration_type, p_assignable->identifier->name);
			}
		} else {
			parser->push_warning(p_assignable, OScriptWarning::UNTYPED_DECLARATION, declaration_type, p_assignable->identifier->name);
		}
	} else if (!is_parameter && specified_type.kind == OScriptParser::DataType::ENUM && p_assignable->initializer == nullptr) {
		...
		if (!has_zero_value) {
			parser->push_warning(p_assignable, OScriptWarning::ENUM_VARIABLE_WITHOUT_DEFAULT, p_assignable->identifier->name);
		}
	}
    #endif // DEBUG_ENABLED
...
void OScriptAnalyzer::resolve_variable(OScriptParser::VariableNode* p_variable, bool p_is_local) {
    ...
    if (p_is_local) {
        if (p_variable->usages == 0 && !String(p_variable->identifier->name).begins_with("_")) {
            parser->push_warning(p_variable, OScriptWarning::UNUSED_VARIABLE, p_variable->identifier->name);
        }
    }
...
void OScriptAnalyzer::resolve_constant(OScriptParser::ConstantNode* p_constant, bool p_is_local) {
    ...
    if (p_is_local) {
        if (p_constant->usages == 0 && !String(p_constant->identifier->name).begins_with("_")) {
            parser->push_warning(p_constant, OScriptWarning::UNUSED_LOCAL_CONSTANT, p_constant->identifier->name);
        }
    }
...
void OScriptAnalyzer::resolve_parameter(OScriptParser::ParameterNode* p_parameter) {
    static constexpr const char* kind = "parameter";
    resolve_assignable(p_parameter, kind);
}

void OScriptAnalyzer::resolve_if(OScriptParser::IfNode* p_if) {
...
void OScriptAnalyzer::resolve_for(OScriptParser::ForNode* p_for) {
    OScriptParser::DataType variable_type;
	...
	if (p_for->list) {
		...
		if (p_for->list->type == OScriptParser::Node::CALL) {
			OScriptParser::CallNode* call = static_cast<OScriptParser::CallNode*>(p_for->list);
			if (call->get_callee_type() == OScriptParser::Node::IDENTIFIER) {
				if (static_cast<OScriptParser::IdentifierNode*>(call->callee)->name == StringName("range")) {
					...
					is_range = true;
					variable_type.type_source = OScriptParser::DataType::ANNOTATED_INFERRED;
					variable_type.kind = OScriptParser::DataType::BUILTIN;
		...
		if (is_range) {
			...
			variable_type.kind = OScriptParser::DataType::VARIANT;
			mark_node_unsafe(p_for->list);
			...
			OScriptParser::DataType return_type;
			List<OScriptParser::DataType> par_types;
	...
	if (p_for->variable) {
		if (p_for->datatype_specifier) {
			OScriptParser::DataType specified_type = type_from_metatype(resolve_datatype(p_for->datatype_specifier));
			if (!specified_type.is_variant()) {
				...
				if (p_for->list) {
					if (p_for->list->type == OScriptParser::Node::ARRAY) {
						update_array_literal_element_type(static_cast<OScriptParser::ArrayNode*>(p_for->list), specified_type);
					} else if (p_for->list->type == OScriptParser::Node::DICTIONARY) {
						update_dictionary_literal_element_type(static_cast<OScriptParser::DictionaryNode*>(p_for->list), specified_type, OScriptParser::DataType::get_variant_type());
					}
				}
			}
			...
			if (variable_type.is_hard_type()) {
				parser->push_warning(p_for->variable, OScriptWarning::INFERRED_DECLARATION, R"("for" iterator variable)", p_for->variable->name);
			} else {
				parser->push_warning(p_for->variable, OScriptWarning::UNTYPED_DECLARATION, R"("for" iterator variable)", p_for->variable->name);
			}
            #endif // DEBUG_ENABLED
		}
	}
...
void OScriptAnalyzer::resolve_assert(OScriptParser::AssertNode* p_assert) {
    ...
    if (p_assert->message != nullptr) {
        ...
        if (!p_assert->message->get_datatype().has_no_type() &&
            (p_assert->message->get_datatype().kind != OScriptParser::DataType::BUILTIN ||
    ...
    if (p_assert->condition->is_constant) {
        if (p_assert->condition->reduced_value.booleanize()) {
            parser->push_warning(p_assert->condition, OScriptWarning::ASSERT_ALWAYS_TRUE);
        } else if (!(p_assert->condition->type == OScriptParser::Node::LITERAL &&
            static_cast<OScriptParser::LiteralNode*>(p_assert->condition)->value.get_type() == Variant::BOOL)) {
            parser->push_warning(p_assert->condition, OScriptWarning::ASSERT_ALWAYS_FALSE);
        }
    }
    #endif // DEBUG_ENABLED
}

void OScriptAnalyzer::resolve_match(OScriptParser::MatchNode* p_match) {
...
void OScriptAnalyzer::resolve_match_branch(OScriptParser::MatchBranchNode* p_match_branch, OScriptParser::ExpressionNode* p_match_test) {
    // Apply annotations.
    for (OScriptParser::AnnotationNode*& E : p_match_branch->annotations) {
        resolve_annotation(E);
...
void OScriptAnalyzer::resolve_match_pattern(OScriptParser::PatternNode* p_pattern, OScriptParser::ExpressionNode* p_match_test) {
	...
	switch (p_pattern->pattern_type) {
		case OScriptParser::PatternNode::PT_LITERAL: {
		    if (p_pattern->literal) {
		        reduce_literal(p_pattern->literal);
		        result = p_pattern->literal->get_datatype();
		    }
		    break;
		}
		case OScriptParser::PatternNode::PT_EXPRESSION: {
		    if (p_pattern->expression) {
		        OScriptParser::ExpressionNode* expr = p_pattern->expression;
		        reduce_expression(expr);
		        result = expr->get_datatype();
		        if (!expr->is_constant) {
		            while (expr && expr->type == OScriptParser::Node::SUBSCRIPT) {
		                OScriptParser::SubscriptNode* sub = static_cast<OScriptParser::SubscriptNode* >(expr);
		                if (!sub->is_attribute) {
		                    expr = nullptr;
		                } else {
		                    expr = sub->base;
		            ...
		            if (!expr || expr->type != OScriptParser::Node::IDENTIFIER) {
		...
		case OScriptParser::PatternNode::PT_BIND: {
		    ...
		    p_pattern->bind->set_datatype(result);
		    #ifdef DEBUG_ENABLED
		    is_shadowing(p_pattern->bind, "pattern bind", true);
		    if (p_pattern->bind->usages == 0 && !String(p_pattern->bind->name).begins_with("_")) {
		        parser->push_warning(p_pattern->bind, OScriptWarning::UNUSED_VARIABLE, p_pattern->bind->name);
		    }
		    #endif // DEBUG_ENABLED
		    break;
		}
		case OScriptParser::PatternNode::PT_ARRAY: {
		    for (int i = 0; i < p_pattern->array.size(); i++) {
		        resolve_match_pattern(p_pattern->array[i], nullptr);
		        decide_suite_type(p_pattern, p_pattern->array[i]);
		    }
		    result = p_pattern->get_datatype();
		    break;
		}
		case OScriptParser::PatternNode::PT_DICTIONARY: {
		    for (int i = 0; i < p_pattern->dictionary.size(); i++) {
		        if (p_pattern->dictionary[i].key) {
		            reduce_expression(p_pattern->dictionary[i].key);
		            if (!p_pattern->dictionary[i].key->is_constant) {
		                push_error(R"(Expression in dictionary pattern key must be a constant.)", p_pattern->dictionary[i].key);
		            }
		        }

		        if (p_pattern->dictionary[i].value_pattern) {
		            resolve_match_pattern(p_pattern->dictionary[i].value_pattern, nullptr);
		            decide_suite_type(p_pattern, p_pattern->dictionary[i].value_pattern);
		        }
		    }
		    result = p_pattern->get_datatype();
		    break;
...
void OScriptAnalyzer::resolve_return(OScriptParser::ReturnNode* p_return) {
	...
	if (p_return->return_value != nullptr) {
		...
		if (is_void_function) {
			...
			const OScriptParser::DataType& return_type = p_return->return_value->data_type;

			if (is_call && !return_type.is_hard_type()) {
				String function_name = parser->current_function->identifier ? String(parser->current_function->identifier->name) : String("<anonymous function>");
				String called_function_name = static_cast<OScriptParser::CallNode*>(p_return->return_value)->function_name;
                #ifdef DEBUG_ENABLED
				parser->push_warning(p_return, OScriptWarning::UNSAFE_VOID_RETURN, function_name, called_function_name);
                #endif // DEBUG_ENABLED
			...
			if (p_return->return_value->type == OScriptParser::Node::ARRAY && has_expected_type && expected_type.has_container_element_type(0)) {
				update_array_literal_element_type(static_cast<OScriptParser::ArrayNode*>(p_return->return_value), expected_type.get_container_element_type(0));
			} else if (p_return->return_value->type == OScriptParser::Node::DICTIONARY && has_expected_type && expected_type.has_container_element_types()) {
				update_dictionary_literal_element_type(static_cast<OScriptParser::DictionaryNode*>(p_return->return_value),
...
void OScriptAnalyzer::mark_lambda_use_self() {
    OScriptParser::LambdaNode* lambda = current_lambda;
...
void OScriptAnalyzer::resolve_pending_lambda_bodies() {
    if (pending_body_resolution_lambdas.is_empty()) {
        return;
    }
    ...
    for (OScriptParser::LambdaNode* lambda : lambdas) {
        ...
        if (captures_amount > 0) {
            ...
            for (int i = 0; i < lambda->captures.size(); i++) {
                OScriptParser::IdentifierNode* capture = lambda->captures[i];
                OScriptParser::ParameterNode* capture_param = parser->alloc_node<OScriptParser::ParameterNode>();
...
void OScriptAnalyzer::reduce_expression(OScriptParser::ExpressionNode* p_expression, bool p_is_root) {
	...
	switch (p_expression->type) {
		case OScriptParser::Node::ARRAY: {
		    reduce_array(static_cast<OScriptParser::ArrayNode*>(p_expression));
		    break;
		}
		case OScriptParser::Node::ASSIGNMENT: {
		    reduce_assignment(static_cast<OScriptParser::AssignmentNode*>(p_expression));
		    break;
		}
		case OScriptParser::Node::AWAIT: {
		    reduce_await(static_cast<OScriptParser::AwaitNode*>(p_expression));
		    break;
		}
		case OScriptParser::Node::BINARY_OPERATOR: {
		    reduce_binary_op(static_cast<OScriptParser::BinaryOpNode*>(p_expression));
		    break;
		}
		case OScriptParser::Node::CALL: {
		    reduce_call(static_cast<OScriptParser::CallNode*>(p_expression), false, p_is_root);
		    break;
		}
		case OScriptParser::Node::CAST: {
		    reduce_cast(static_cast<OScriptParser::CastNode*>(p_expression));
		    break;
		}
		case OScriptParser::Node::DICTIONARY: {
		    reduce_dictionary(static_cast<OScriptParser::DictionaryNode*>(p_expression));
		    break;
		}
		case OScriptParser::Node::GET_NODE: {
		    reduce_get_node(static_cast<OScriptParser::GetNodeNode*>(p_expression));
		    break;
		}
		case OScriptParser::Node::IDENTIFIER: {
		    reduce_identifier(static_cast<OScriptParser::IdentifierNode*>(p_expression));
		    break;
		}
		case OScriptParser::Node::LAMBDA: {
		    reduce_lambda(static_cast<OScriptParser::LambdaNode*>(p_expression));
		    break;
		}
		case OScriptParser::Node::LITERAL: {
		    reduce_literal(static_cast<OScriptParser::LiteralNode*>(p_expression));
		    break;
		}
		case OScriptParser::Node::PRELOAD: {
		    reduce_preload(static_cast<OScriptParser::PreloadNode*>(p_expression));
		    break;
		}
		case OScriptParser::Node::SELF: {
		    reduce_self(static_cast<OScriptParser::SelfNode*>(p_expression));
		    break;
		}
		case OScriptParser::Node::SUBSCRIPT: {
		    reduce_subscript(static_cast<OScriptParser::SubscriptNode*>(p_expression));
		    break;
		}
		case OScriptParser::Node::TERNARY_OPERATOR: {
		    reduce_ternary_op(static_cast<OScriptParser::TernaryOpNode*>(p_expression), p_is_root);
		    break;
		}
		case OScriptParser::Node::TYPE_TEST: {
		    reduce_type_test(static_cast<OScriptParser::TypeTestNode*>(p_expression));
		    break;
		}
		case OScriptParser::Node::UNARY_OPERATOR: {
		    reduce_unary_op(static_cast<OScriptParser::UnaryOpNode*>(p_expression));
		    break;
		}
	...
	if (p_expression->get_datatype().kind == OScriptParser::DataType::UNRESOLVED) {
		// Prevent `is_type_compatible()` errors for incomplete expressions.
		// The error can still occur if `reduce_*()` is called directly.
		OScriptParser::DataType dummy;
		dummy.kind = OScriptParser::DataType::VARIANT;
		p_expression->set_datatype(dummy);
	}
}

void OScriptAnalyzer::reduce_array(OScriptParser::ArrayNode* p_array) {
    for (int i = 0; i < p_array->elements.size(); i++) {
        OScriptParser::ExpressionNode* element = p_array->elements[i];
        reduce_expression(element);
    }

    // It's array in any case.
    OScriptParser::DataType arr_type;
    arr_type.type_source = OScriptParser::DataType::ANNOTATED_EXPLICIT;
    arr_type.kind = OScriptParser::DataType::BUILTIN;
...
void OScriptAnalyzer::reduce_assignment(OScriptParser::AssignmentNode* p_assignment) {
	...
	if (p_assignment->assignee->type == OScriptParser::Node::IDENTIFIER) {
		OScriptParser::IdentifierNode *id = static_cast<OScriptParser::IdentifierNode*>(p_assignment->assignee);
		if (id->source == OScriptParser::IdentifierNode::LOCAL_VARIABLE && id->variable_source) {
    ...
    #ifdef DEBUG_ENABLED
	{
		...
		OScriptParser::ExpressionNode*base = p_assignment->assignee;
		while (base && base->type == OScriptParser::Node::SUBSCRIPT) {
			is_subscript = true;
			base = static_cast<OScriptParser::SubscriptNode*>(base)->base;
		}

		if (base && base->type == OScriptParser::Node::IDENTIFIER) {
			OScriptParser::IdentifierNode *id = static_cast<OScriptParser::IdentifierNode*>(base);
			if (current_lambda && current_lambda->captures_indices.has(id->name)) {
				bool need_warn = false;
				if (is_subscript) {
					const OScriptParser::DataType &id_type = id->data_type;
				...
				if (need_warn) {
					parser->push_warning(p_assignment, OScriptWarning::CONFUSABLE_CAPTURE_REASSIGNMENT, id->name);
				}
			}
		}
	}
	...
	OScriptParser::DataType assignee_type = p_assignment->assignee->get_datatype();

	if (assignee_type.is_constant) {
		push_error("Cannot assign a new value to a constant.", p_assignment->assignee);
		return;
	} else if (p_assignment->assignee->type == OScriptParser::Node::SUBSCRIPT && static_cast<OScriptParser::SubscriptNode*>(p_assignment->assignee)->base->is_constant) {
		const OScriptParser::DataType& base_type = static_cast<OScriptParser::SubscriptNode*>(p_assignment->assignee)->base->data_type;
		if (base_type.kind != OScriptParser::DataType::SCRIPT && base_type.kind != OScriptParser::DataType::CLASS) { // Static variables.
			push_error("Cannot assign a new value to a constant.", p_assignment->assignee);
			return;
		}
		...
		return;
	} else if (p_assignment->assignee->type == OScriptParser::Node::SUBSCRIPT) {
		OScriptParser::SubscriptNode *sub = static_cast<OScriptParser::SubscriptNode*>(p_assignment->assignee);
		while (sub) {
			const OScriptParser::DataType& base_type = sub->base->data_type;
			if (base_type.is_hard_type() && base_type.is_read_only) {
				if (base_type.kind == OScriptParser::DataType::BUILTIN && !GDE::Variant::is_type_shared(base_type.builtin_type)) {
					push_error("Cannot assign a new value to a read-only property.", p_assignment->assignee);
					return;
				}
			} else {
			...
			if (sub->base->type == OScriptParser::Node::SUBSCRIPT) {
				sub = static_cast<OScriptParser::SubscriptNode*>(sub->base);
			} else {
				sub = nullptr;
			}
		}
	...
	if (p_assignment->assigned_value->type == OScriptParser::Node::ARRAY && assignee_type.is_hard_type() && assignee_type.has_container_element_type(0)) {
		update_array_literal_element_type(static_cast<OScriptParser::ArrayNode*>(p_assignment->assigned_value), assignee_type.get_container_element_type(0));
	} else if (p_assignment->assigned_value->type == OScriptParser::Node::DICTIONARY && assignee_type.is_hard_type() && assignee_type.has_container_element_types()) {
		update_dictionary_literal_element_type(static_cast<OScriptParser::DictionaryNode*>(p_assignment->assigned_value),
				assignee_type.get_container_element_type_or_variant(0), assignee_type.get_container_element_type_or_variant(1));
	}

	if (p_assignment->operation == OScriptParser::AssignmentNode::OP_NONE && assignee_type.is_hard_type() && p_assignment->assigned_value->is_constant) {
		update_const_expression_builtin_type(p_assignment->assigned_value, assignee_type, "assign");
	}

	OScriptParser::DataType assigned_value_type = p_assignment->assigned_value->get_datatype();
	...
	OScriptParser::DataType op_type = assigned_value_type;

	if (p_assignment->operation != OScriptParser::AssignmentNode::OP_NONE && !op_type.is_variant()) {
		...
		if (assignee_is_variant) {
			...
			if (assigned_is_variant) {
				...
				push_error(vformat(R"(Invalid operands "%s" and "%s" for assignment operator.)",
				    assignee_type.to_string(), assigned_value_type.to_string()), p_assignment);
			}
		} else if (op_type.type_source == OScriptParser::DataType::UNDETECTED && !assigned_is_variant) {
	...
	if (assignee_is_variant) {
		...
		if (assignee_is_hard && !assigned_is_hard) {
			...
			if (op_type.is_variant()) {
				...
				if (assignee_is_hard) {
					if (is_type_compatible(op_type, assignee_type)) {
						...
						push_error(vformat(R"(Value of type "%s" cannot be assigned to a variable of type "%s".)",
						    assigned_value_type.to_string(), assignee_type.to_string()), p_assignment->assigned_value);
					}
				} else {
			    ...
			    (assignee_type.has_container_element_type(1) && !op_type.has_container_element_type(1))) {
	...
	if (assignee_type.is_hard_type() && assignee_type.builtin_type == Variant::INT && assigned_value_type.builtin_type == Variant::FLOAT) {
		parser->push_warning(p_assignment->assigned_value, OScriptWarning::NARROWING_CONVERSION);
	}

	// Check for assignment with operation before assignment.
	if (p_assignment->operation != OScriptParser::AssignmentNode::OP_NONE && p_assignment->assignee->type == OScriptParser::Node::IDENTIFIER) {
		OScriptParser::IdentifierNode* id = static_cast<OScriptParser::IdentifierNode*>(p_assignment->assignee);

	    // Use == 1 here because this assignment was already counted in the beginning of the function.
		if (id->source == OScriptParser::IdentifierNode::LOCAL_VARIABLE && id->variable_source && id->variable_source->assignments == 1) {
			parser->push_warning(p_assignment, OScriptWarning::UNASSIGNED_VARIABLE_OP_ASSIGN, id->name, GDE::Variant::get_operator_name(p_assignment->variant_op));
		}
	}
    #endif // DEBUG_ENABLED
}

void OScriptAnalyzer::reduce_await(OScriptParser::AwaitNode* p_await) {
    if (p_await->to_await == nullptr) {
        OScriptParser::DataType await_type;
        await_type.kind = OScriptParser::DataType::VARIANT;
        p_await->set_datatype(await_type);
        return;
    }

    if (p_await->to_await->type == OScriptParser::Node::CALL) {
        reduce_call(static_cast<OScriptParser::CallNode*>(p_await->to_await), true);
    } else {
        reduce_expression(p_await->to_await);
    }

    OScriptParser::DataType await_type = p_await->to_await->get_datatype();
    // We cannot infer the type of the result of waiting for a signal.
    if (await_type.is_hard_type() && await_type.kind == OScriptParser::DataType::BUILTIN && await_type.builtin_type == Variant::SIGNAL) {
        await_type.kind = OScriptParser::DataType::VARIANT;
        await_type.type_source = OScriptParser::DataType::UNDETECTED;
    ...
    #ifdef DEBUG_ENABLED
    OScriptParser::DataType to_await_type = p_await->to_await->get_datatype();
    if (!to_await_type.is_coroutine && !to_await_type.is_variant() && to_await_type.builtin_type != Variant::SIGNAL) {
        parser->push_warning(p_await, OScriptWarning::REDUNDANT_AWAIT);
    }
    #endif // DEBUG_ENABLED
}

void OScriptAnalyzer::reduce_binary_op(OScriptParser::BinaryOpNode* p_binary_op) {
    reduce_expression(p_binary_op->left_operand);
	reduce_expression(p_binary_op->right_operand);

	OScriptParser::DataType left_type;
	if (p_binary_op->left_operand) {
		left_type = p_binary_op->left_operand->get_datatype();
	}

	OScriptParser::DataType right_type;
	...
	if (p_binary_op->variant_op == Variant::OP_DIVIDE &&
		...
		parser->push_warning(p_binary_op, OScriptWarning::INTEGER_DIVISION);
	}
    #endif // DEBUG_ENABLED
	...
	if (p_binary_op->left_operand->is_constant && p_binary_op->right_operand->is_constant) {
		...
		if (p_binary_op->variant_op < Variant::OP_MAX) {
			...
			if (!valid) {
			    const String operator_name = GDE::Variant::get_operator_name(p_binary_op->variant_op);
				if (p_binary_op->reduced_value.get_type() == Variant::STRING) {
					push_error(vformat(R"(%s in operator %s.)", p_binary_op->reduced_value, operator_name), p_binary_op);
				} else {
	...
	if ((p_binary_op->variant_op == Variant::OP_EQUAL || p_binary_op->variant_op == Variant::OP_NOT_EQUAL) &&
			((left_type.kind == OScriptParser::DataType::BUILTIN && left_type.builtin_type == Variant::NIL) || (right_type.kind == OScriptParser::DataType::BUILTIN && right_type.builtin_type == Variant::NIL))) {
		...
		if (!valid) {
		    const StringName operator_name = GDE::Variant::get_operator_name(p_binary_op->variant_op);
			push_error(vformat(R"(Invalid operands "%s" and "%s" for "%s" operator.)", left_type.to_string(), right_type.to_string(), operator_name), p_binary_op);
...
void OScriptAnalyzer::reduce_call(OScriptParser::CallNode* p_call, bool p_is_await, bool p_is_root) {
    bool all_is_constant = true;
	HashMap<int, OScriptParser::ArrayNode*> arrays; // For array literal to potentially type when passing.
	HashMap<int, OScriptParser::DictionaryNode*> dictionaries; // Same, but for dictionaries.

	for (int i = 0; i < p_call->arguments.size(); i++) {
		reduce_expression(p_call->arguments[i]);
	    if (p_call->arguments[i] == nullptr) {
	        push_error(R"*(Call argument resolved as null, potential parser bug?)*", p_call);
	        return;
	    }
		if (p_call->arguments[i]->type == OScriptParser::Node::ARRAY) {
			arrays[i] = static_cast<OScriptParser::ArrayNode*>(p_call->arguments[i]);
		} else if (p_call->arguments[i]->type == OScriptParser::Node::DICTIONARY) {
			dictionaries[i] = static_cast<OScriptParser::DictionaryNode*>(p_call->arguments[i]);
		}
		all_is_constant = all_is_constant && p_call->arguments[i]->is_constant;
	}

	OScriptParser::Node::Type callee_type = p_call->get_callee_type();
	OScriptParser::DataType call_type;

	if (!p_call->is_super && callee_type == OScriptParser::Node::IDENTIFIER) {
		// Call to name directly.
		StringName function_name = p_call->function_name;

		if (function_name == StringName("Object")) {
			push_error(R"*(Invalid constructor "Object()", use "Object.new()" instead.)*", p_call);
			p_call->set_datatype(call_type);
			return;
		}

		Variant::Type builtin_type = OScriptParser::get_builtin_type(function_name);
		if (builtin_type < Variant::VARIANT_MAX) {
			// Is a builtin constructor.
			call_type.type_source = OScriptParser::DataType::ANNOTATED_EXPLICIT;
			...
			if (all_is_constant && safe_to_fold) {
			    ...
			    Variant value;
			    GDExtensionCallError err = GDE::Variant::construct(builtin_type, value, (const Variant**)args.ptr(), args.size());

				switch (err.error) {
				    case GDEXTENSION_CALL_ERROR_INVALID_ARGUMENT: {
				        push_error(vformat(R"*(Invalid argument for "%s()" constructor: argument %d should be "%s" but is "%s".)*",
				            Variant::get_type_name(builtin_type), err.argument + 1, Variant::get_type_name(Variant::Type(err.expected)),
				            p_call->arguments[err.argument]->get_datatype().to_string()),
					...
					case GDEXTENSION_CALL_ERROR_INVALID_METHOD: {
						...
						push_error(vformat(R"(No constructor of "%s" matches the signature "%s".)",
						    Variant::get_type_name(builtin_type), signature), p_call->callee);
				        break;
					}
				    case GDEXTENSION_CALL_ERROR_TOO_MANY_ARGUMENTS: {
				        push_error(vformat(R"*(Too many arguments for "%s()" constructor. Received %d but expected %d.)*",
				            Variant::get_type_name(builtin_type), p_call->arguments.size(), err.expected), p_call);
				        break;
				    }
				    case GDEXTENSION_CALL_ERROR_TOO_FEW_ARGUMENTS: {
				        push_error(vformat(R"*(Too few arguments for "%s()" constructor. Received %d but expected %d.)*",
				            Variant::get_type_name(builtin_type), p_call->arguments.size(), err.expected), p_call);
				        break;
				    }
				...
				if (p_call->arguments.size() == 1) {
					OScriptParser::DataType arg_type = p_call->arguments[0]->get_datatype();
					if (arg_type.is_hard_type() && !arg_type.is_variant()) {
						if (arg_type.kind == OScriptParser::DataType::BUILTIN && arg_type.builtin_type == builtin_type) {
							// Okay.
							p_call->set_datatype(call_type);
							return;
						}
						...
						parser->push_warning(p_call->arguments[0], OScriptWarning::UNSAFE_CALL_ARGUMENT, "1", "constructor", function_name, expected_types, "Variant");
                        #endif // DEBUG_ENABLED
						p_call->set_datatype(call_type);
						return;
					}
			    ...
			    for (const MethodInfo &info : constructors) {
					...
					for (int64_t i = 0; i < p_call->arguments.size(); ++i) {
						OScriptParser::DataType par_type = type_from_property(info.arguments[i], true);
						OScriptParser::DataType arg_type = p_call->arguments[i]->get_datatype();
						if (!is_type_compatible(par_type, arg_type, true)) {
							...
							if (par_type.builtin_type == Variant::INT && arg_type.builtin_type == Variant::FLOAT && builtin_type != Variant::INT) {
								parser->push_warning(p_call, OScriptWarning::NARROWING_CONVERSION, function_name);
							}
                        #endif // DEBUG_ENABLED
						}
					}
						...
						for (int64_t i = 0; i < p_call->arguments.size(); ++i) {
							...
							if (!(par_type.is_variant() && par_type.is_hard_type())) {
								...
								if (arg_type.is_variant() || !arg_type.is_hard_type()) {
									mark_node_unsafe(p_call);
									parser->push_warning(p_call->arguments[i], OScriptWarning::UNSAFE_CALL_ARGUMENT, itos(i + 1),
									    "constructor", function_name, par_type.to_string(), arg_type.to_string_strict());
								}
							}
                            #endif // DEBUG_ENABLED
						}
			...
			if (builtin_type == Variant::SIGNAL && p_call->arguments.size() >= 2) {
				const OScriptParser::ExpressionNode* object_arg = p_call->arguments[0];
				if (object_arg && object_arg->type == OScriptParser::Node::SELF) {
					const OScriptParser::ExpressionNode* signal_arg = p_call->arguments[1];
					if (signal_arg && signal_arg->is_constant) {
						...
						if (parser->current_class->has_member(signal_name)) {
							const OScriptParser::ClassNode::Member &member = parser->current_class->get_member(signal_name);
							if (member.type == OScriptParser::ClassNode::Member::SIGNAL) {
		...
		} else if (OScriptUtilityFunctions::function_exists(function_name)) {
			const MethodInfo function_info = OScriptUtilityFunctions::get_function_info(function_name);
			...
			if (all_is_constant && OScriptUtilityFunctions::is_function_constant(function_name)) {
				...
				OScriptUtilityFunctions::get_function(function_name)(&value, (const Variant **)args.ptr(), args.size(), err);
				...
				switch (err.error) {
				    ...
				    case GDEXTENSION_CALL_ERROR_TOO_MANY_ARGUMENTS: {
				        push_error(vformat(R"*(Too many arguments for "%s()" call. Expected at most %d but received %d.)*",
				            function_name, err.expected, p_call->arguments.size()), p_call);
				        break;
				    }
				    case GDEXTENSION_CALL_ERROR_TOO_FEW_ARGUMENTS: {
				        push_error(vformat(R"*(Too few arguments for "%s()" call. Expected at least %d but received %d.)*",
				            function_name, err.expected, p_call->arguments.size()), p_call);
				        break;
				    }
				    case GDEXTENSION_CALL_ERROR_METHOD_NOT_CONST:
		...
		} else if (GDE::Variant::has_utility_function(function_name)) {
			MethodInfo function_info = GDE::Variant::get_utility_function_method_info(function_name);

			if (!p_is_root && !p_is_await && function_info.return_val.type == Variant::NIL && ((function_info.return_val.usage & PROPERTY_USAGE_NIL_IS_VARIANT) == 0)) {
				push_error(vformat(R"*(Cannot get return value of call to "%s()" because it returns "void".)*", function_name), p_call);
			}

			if (all_is_constant && GDE::Variant::get_utility_function_type(function_name) == GDE::Variant::UTILITY_FUNC_TYPE_MATH) {
				// Can call on compilation.
				Vector<const Variant*> args;
				for (int i = 0; i < p_call->arguments.size(); i++) {
					args.push_back(&(p_call->arguments[i]->reduced_value));
				...
				Variant value;
			    GDExtensionCallError err;
			    GDE::Variant::call_utility_function(function_name, value, (const Variant**) args.ptr(), args.size(), err);
	...
	call_type.kind = OScriptParser::DataType::VARIANT;
	bool is_self = false;
	...
	if (p_call->is_super) {
	...
	} else if (callee_type == OScriptParser::Node::IDENTIFIER) {
		base_type = parser->current_class->get_datatype();
		base_type.is_meta_type = false;
		is_self = true;
	} else if (callee_type == OScriptParser::Node::SUBSCRIPT) {
		OScriptParser::SubscriptNode* subscript = static_cast<OScriptParser::SubscriptNode*>(p_call->callee);
		...
		OScriptParser::IdentifierNode* base_id = nullptr;
		if (subscript->base->type == OScriptParser::Node::IDENTIFIER) {
			base_id = static_cast<OScriptParser::IdentifierNode*>(subscript->base);
		}
		if (base_id && OScriptParser::get_builtin_type(base_id->name) < Variant::VARIANT_MAX) {
			base_type = make_builtin_meta_type(OScriptParser::get_builtin_type(base_id->name));
		} else {
			reduce_expression(subscript->base);
			base_type = subscript->base->get_datatype();
			is_self = subscript->base->type == OScriptParser::Node::SELF;
		}
	} else {
		// Invalid call. Error already sent in parser.
	...
	bool is_constructor = (base_type.is_meta_type || (p_call->callee && p_call->callee->type == OScriptParser::Node::IDENTIFIER)) && p_call->function_name == StringName("new");
	...
	if (is_constructor) {
		...
		if ((base_type.kind == OScriptParser::DataType::CLASS && base_type.class_type->is_abstract) || (base_type.kind == OScriptParser::DataType::SCRIPT && base_type.script_type.is_valid() && base_type.script_type->is_abstract())) {
	...
	if (get_function_signature(p_call, is_constructor, base_type, p_call->function_name, return_type, par_types, default_arg_count, method_flags)) {
		...
		for (const KeyValue<int, OScriptParser::ArrayNode*>& E : arrays) {
			int index = E.key;
		...
		for (const KeyValue<int, OScriptParser::DictionaryNode*>& E : dictionaries) {
			int index = E.key;
			if (index < par_types.size() && par_types.get(index).is_hard_type() && par_types.get(index).has_container_element_types()) {
				OScriptParser::DataType key = par_types.get(index).get_container_element_type_or_variant(0);
				OScriptParser::DataType value = par_types.get(index).get_container_element_type_or_variant(1);
		...
		if (base_type.kind == OScriptParser::DataType::ENUM && base_type.is_meta_type) {
		...
		if (is_self && static_context && !p_call->is_static) {
			...
			OScriptParser::FunctionNode* parent_function = parser->current_function;
		...
		if (!p_is_root && !p_is_await && return_type.is_hard_type() && return_type.kind == OScriptParser::DataType::BUILTIN && return_type.builtin_type == Variant::NIL) {
		...
		if (p_is_root && return_type.kind != OScriptParser::DataType::UNRESOLVED && return_type.builtin_type != Variant::NIL &&
				!(p_call->is_super && p_call->function_name == OScriptLanguage::get_singleton()->strings._init)) {
			parser->push_warning(p_call, OScriptWarning::RETURN_VALUE_DISCARDED, p_call->function_name);
		}
		...
		if (method_flags.has_flag(METHOD_FLAG_STATIC) && !is_constructor && !base_type.is_meta_type && !is_self) {
			...
			parser->push_warning(p_call, OScriptWarning::STATIC_CALLED_ON_INSTANCE, p_call->function_name, caller_type);
		}
		...
		if (is_self && (p_call->function_name == StringName("emit_signal") || p_call->function_name == StringName("connect") || p_call->function_name == StringName("disconnect")) && !p_call->arguments.is_empty()) {
			const OScriptParser::ExpressionNode* signal_arg = p_call->arguments[0];
			if (signal_arg && signal_arg->is_constant) {
				const StringName &signal_name = signal_arg->reduced_value;
				if (parser->current_class->has_member(signal_name)) {
					const OScriptParser::ClassNode::Member& member = parser->current_class->get_member(signal_name);
		...
		} else if (!p_call->is_super && callee_type != OScriptParser::Node::NONE) { // Check if the name exists as something else.
			OScriptParser::IdentifierNode *callee_id;
			if (callee_type == OScriptParser::Node::IDENTIFIER) {
				callee_id = static_cast<OScriptParser::IdentifierNode*>(p_call->callee);
			} else {
				// Can only be attribute.
				callee_id = static_cast<OScriptParser::SubscriptNode*>(p_call->callee)->attribute;
			}
			if (callee_id) {
				reduce_identifier_from_base(callee_id, &base_type);
				OScriptParser::DataType callee_datatype = callee_id->get_datatype();
                ...
                #ifdef DEBUG_ENABLED
				} else if (!is_self && !(base_type.is_hard_type() && base_type.kind == OScriptParser::DataType::BUILTIN)) {
					parser->push_warning(p_call, OScriptWarning::UNSAFE_METHOD_ACCESS, p_call->function_name, base_type.to_string());
					mark_node_unsafe(p_call);
                #endif // DEBUG_ENABLED
				}
			}
		}
		if (!found && (is_self || (base_type.is_hard_type() && base_type.kind == OScriptParser::DataType::BUILTIN))) {
	...
	if (call_type.is_coroutine && !p_is_await) {
		if (p_is_root) {
            #ifdef DEBUG_ENABLED
			parser->push_warning(p_call, OScriptWarning::MISSING_AWAIT);
            #endif // DEBUG_ENABLED
		} else {
...
void OScriptAnalyzer::reduce_cast(OScriptParser::CastNode* p_cast) {
	...
	if (p_cast->operand->type == OScriptParser::Node::ARRAY && cast_type.has_container_element_type(0)) {
		update_array_literal_element_type(static_cast<OScriptParser::ArrayNode*>(p_cast->operand), cast_type.get_container_element_type(0));
	}

	if (p_cast->operand->type == OScriptParser::Node::DICTIONARY && cast_type.has_container_element_types()) {
		update_dictionary_literal_element_type(static_cast<OScriptParser::DictionaryNode*>(p_cast->operand),
				cast_type.get_container_element_type_or_variant(0), cast_type.get_container_element_type_or_variant(1));
	}

	if (!cast_type.is_variant()) {
		OScriptParser::DataType op_type = p_cast->operand->get_datatype();
		if (op_type.is_variant() || !op_type.is_hard_type()) {
			mark_node_unsafe(p_cast);
            #ifdef DEBUG_ENABLED
			parser->push_warning(p_cast, OScriptWarning::UNSAFE_CAST, cast_type.to_string());
            #endif // DEBUG_ENABLED
		} else {
			bool valid = false;
			if (op_type.builtin_type == Variant::INT && cast_type.kind == OScriptParser::DataType::ENUM) {
				mark_node_unsafe(p_cast);
				valid = true;
			} else if (op_type.kind == OScriptParser::DataType::ENUM && cast_type.builtin_type == Variant::INT) {
				valid = true;
			} else if (op_type.kind == OScriptParser::DataType::BUILTIN && cast_type.kind == OScriptParser::DataType::BUILTIN) {
...
void OScriptAnalyzer::reduce_dictionary(OScriptParser::DictionaryNode* p_dictionary) {
    ...
    for (int i = 0; i < p_dictionary->elements.size(); i++) {
        ...
        if (p_dictionary->style == OScriptParser::DictionaryNode::PYTHON_DICT) {
        ...
        if (element.key->is_constant) {
            if (elements.has(element.key->reduced_value)) {
                push_error(vformat(R"(Key "%s" was already used in this dictionary (at line %d).)",
                    element.key->reduced_value, elements[element.key->reduced_value]->script_node_id), element.key);
            } else {
    ...
    OScriptParser::DataType dict_type;
    dict_type.type_source = OScriptParser::DataType::ANNOTATED_EXPLICIT;
    dict_type.kind = OScriptParser::DataType::BUILTIN;
...
void OScriptAnalyzer::reduce_get_node(OScriptParser::GetNodeNode* p_get_node) {
    OScriptParser::DataType result;
    result.kind = OScriptParser::DataType::VARIANT;

    if (!ClassDB::is_parent_class(parser->current_class->base_type.native_type, StringName("Node"))) {
        push_error(vformat(R"*(Cannot use shorthand "get_node()" notation ("%c") on a class that isn't a node.)*",
            p_get_node->use_dollar ? '$' : '%'), p_get_node);
        p_get_node->set_datatype(result);
        return;
    }

    if (static_context) {
        push_error(vformat(R"*(Cannot use shorthand "get_node()" notation ("%c") in a static function.)*",
            p_get_node->use_dollar ? '$' : '%'), p_get_node);
        p_get_node->set_datatype(result);
        return;
    }
...
void OScriptAnalyzer::reduce_identifier(OScriptParser::IdentifierNode* p_identifier, bool p_can_be_builtin) {
	...
	if (current_enum) {
		for (int i = 0; i < current_enum->values.size(); i++) {
			const OScriptParser::EnumNode::Value& element = current_enum->values[i];
			if (element.identifier->name == p_identifier->name) {
				StringName enum_name = current_enum->identifier ? current_enum->identifier->name : UNNAMED_ENUM;
				OScriptParser::DataType type = make_class_enum_type(enum_name, parser->current_class, parser->script_path, false);
	...
	switch (p_identifier->source) {
		...
		case OScriptParser::IdentifierNode::LOCAL_VARIABLE: {
		    ...
		    found_source = true;
		    #ifdef DEBUG_ENABLED
		    if (p_identifier->variable_source && p_identifier->variable_source->assignments == 0 &&
		        !(p_identifier->get_datatype().is_hard_type() && p_identifier->get_datatype().kind == OScriptParser::DataType::BUILTIN)) {
		        parser->push_warning(p_identifier, OScriptWarning::UNASSIGNED_VARIABLE, p_identifier->name);
		    }
		    #endif // DEBUG_ENABLED
		    break;
		}
		...
		case OScriptParser::IdentifierNode::LOCAL_BIND: {
			OScriptParser::DataType result = p_identifier->bind_source->get_datatype();
			result.is_constant = true;
	...
	if (!found_source && p_identifier->suite != nullptr && p_identifier->suite->has_local(p_identifier->name)) {
		parser->push_warning(p_identifier, OScriptWarning::CONFUSABLE_LOCAL_USAGE, p_identifier->name);
	}
    #endif // DEBUG_ENABLED
	...
	if (!found_source) {
		...
		if (p_identifier->source != OScriptParser::IdentifierNode::UNDEFINED_SOURCE || p_identifier->get_datatype().is_set()) {
			// Found.
			found_source = true;
		}
	}
	...
	if (found_source) {
		const bool source_is_instance_variable = p_identifier->source == OScriptParser::IdentifierNode::MEMBER_VARIABLE || p_identifier->source == OScriptParser::IdentifierNode::INHERITED_VARIABLE;
		const bool source_is_instance_function = p_identifier->source == OScriptParser::IdentifierNode::MEMBER_FUNCTION && !p_identifier->function_source_is_static;
		const bool source_is_signal = p_identifier->source == OScriptParser::IdentifierNode::MEMBER_SIGNAL;
		...
		if (static_context && (source_is_instance_variable || source_is_instance_function || source_is_signal)) {
			...
			if (parent_function) {
				push_error(vformat(R"*(Cannot access %s "%s" from the static function "%s()".)*",
				    source_type, p_identifier->name, parent_function->identifier->name), p_identifier);
			} else {
				push_error(vformat(R"*(Cannot access %s "%s" from a static variable initializer.)*",
				    source_type, p_identifier->name), p_identifier);
			}
		}
		...
		if (current_lambda != nullptr) {
			...
			switch (p_identifier->source) {
				...
				case OScriptParser::IdentifierNode::UNDEFINED_SOURCE: // A global.
				case OScriptParser::IdentifierNode::LOCAL_CONSTANT:
				case OScriptParser::IdentifierNode::MEMBER_VARIABLE:
				case OScriptParser::IdentifierNode::MEMBER_CONSTANT:
				case OScriptParser::IdentifierNode::MEMBER_FUNCTION:
			...
			OScriptParser::FunctionNode* function_test = current_lambda->function;
	...
	p_identifier->source = OScriptParser::IdentifierNode::UNDEFINED_SOURCE;

	// Not a local or a member, so check globals.

	Variant::Type builtin_type = OScriptParser::get_builtin_type(name);
	if (builtin_type < Variant::VARIANT_MAX) {
		if (p_can_be_builtin) {
	...
	if (class_exists(name)) {
		p_identifier->source = OScriptParser::IdentifierNode::NATIVE_CLASS;
		...
		if (is_oscript(autoload_path)) {
			...
			if (OScriptLanguage::get_singleton()->has_any_global_constant(name)) {
				...
				if (node != nullptr) {
					Ref<OScript> scr = node->get_script();
					if (scr.is_valid()) {
						Ref<OScriptParserRef> single_parser = parser->get_depended_parser_for(scr->get_script_path());
						if (single_parser.is_valid()) {
							Error err = single_parser->raise_status(OScriptParserRef::INHERITANCE_SOLVED);
							if (err == OK) {
								result = type_from_metatype(single_parser->get_parser()->head->get_datatype());
	...
	if (GDE::CoreConstants::is_global_constant(name)) {
		int index = GDE::CoreConstants::get_global_constant_index(name);
		StringName enum_name = GDE::CoreConstants::get_global_constant_enum(index);
		int64_t value = GDE::CoreConstants::get_global_constant_value(index);
	...
	if (GDE::CoreConstants::is_global_enum(name)) {
	...
	if (GDE::Variant::has_utility_function(name) || OScriptUtilityFunctions::function_exists(name)) {
		p_identifier->is_constant = true;
		p_identifier->reduced_value = Callable(memnew(OScriptUtilityCallable(name)));
		MethodInfo method_info;
		if (OScriptUtilityFunctions::function_exists(name)) {
			method_info = OScriptUtilityFunctions::get_function_info(name);
		} else {
			method_info = GDE::Variant::get_utility_function_method_info(name);
		}
	...
	if (p_can_be_builtin && name == StringName("Variant")) {
		OScriptParser::DataType variant;
		variant.kind = OScriptParser::DataType::VARIANT;
		variant.type_source = OScriptParser::DataType::ANNOTATED_EXPLICIT;
...
void OScriptAnalyzer::reduce_identifier_from_base(OScriptParser::IdentifierNode *p_identifier, OScriptParser::DataType *p_base) {
	...
	if (base.kind == OScriptParser::DataType::ENUM) {
	...
	if (base.kind == OScriptParser::DataType::BUILTIN) {
		if (base.is_meta_type) {
			bool valid = false;

			if (GDE::Variant::has_constant(base.builtin_type, name)) {
				valid = true;

				const Variant constant_value = GDE::Variant::get_constant_value(base.builtin_type, name);
			...
			if (!valid) {
				const StringName enum_name = GDE::Variant::get_enum_for_enumeration(base.builtin_type, name);
				if (enum_name != StringName()) {
					valid = true;

					p_identifier->is_constant = true;
					p_identifier->reduced_value = GDE::Variant::get_enum_value(base.builtin_type, enum_name, name);
					p_identifier->set_datatype(make_builtin_enum_type(enum_name, base.builtin_type, false));
				}
			}

			if (!valid && GDE::Variant::has_enum(base.builtin_type, name)) {
				valid = true;
			...
			switch (base.builtin_type) {
				...
				default: {
				    Variant dummy;
				    GDE::Variant::construct(base.builtin_type, dummy, nullptr, 0);
					Vector<PropertyInfo> properties = GDE::Variant::get_property_list(dummy);
				    ...
				    if (GDE::Variant::has_builtin_method(base.builtin_type, name)) {
						p_identifier->set_datatype(make_callable_type(GDE::Variant::get_builtin_method_info(base.builtin_type, name)));
						return;
					}
	...
	OScriptParser::ClassNode* base_class = base.class_type;
	List<OScriptParser::ClassNode*> script_classes;
	bool is_base = true;
	...
	for (OScriptParser::ClassNode* script_class : script_classes) {
		if (p_base == nullptr && script_class->identifier && script_class->identifier->name == name) {
			...
			if (script_class->outer != nullptr) {
				p_identifier->source = OScriptParser::IdentifierNode::MEMBER_CLASS;
			}
			return;
		}
		...
		if (script_class->has_member(name)) {
		    ...
		    OScriptParser::ClassNode::Member member = script_class->get_member(name);
			switch (member.type) {
				case OScriptParser::ClassNode::Member::CONSTANT: {
					p_identifier->set_datatype(member.get_data_type());
					p_identifier->is_constant = true;
					p_identifier->reduced_value = member.constant->initializer->reduced_value;
					p_identifier->source = OScriptParser::IdentifierNode::MEMBER_CONSTANT;
				...
				case OScriptParser::ClassNode::Member::VARIABLE: {
					if (is_base && (!base.is_meta_type || member.variable->is_static)) {
						p_identifier->set_datatype(member.get_data_type());
						p_identifier->source = member.variable->is_static
					        ? OScriptParser::IdentifierNode::STATIC_VARIABLE
					        : OScriptParser::IdentifierNode::MEMBER_VARIABLE;
				...
				case OScriptParser::ClassNode::Member::SIGNAL: {
					if (is_base && !base.is_meta_type) {
						p_identifier->set_datatype(member.get_data_type());
						p_identifier->source = OScriptParser::IdentifierNode::MEMBER_SIGNAL;
				...
				case OScriptParser::ClassNode::Member::FUNCTION: {
					if (is_base && (!base.is_meta_type || member.function->is_static || is_constructor)) {
						p_identifier->set_datatype(make_callable_type(member.function->method));
						p_identifier->source = OScriptParser::IdentifierNode::MEMBER_FUNCTION;
				...
				case OScriptParser::ClassNode::Member::CLASS: {
					reduce_identifier_from_base_set_class(p_identifier, member.get_data_type());
					p_identifier->source = OScriptParser::IdentifierNode::MEMBER_CLASS;
					return;
				}
				default: {
	...
	if (base_class == nullptr && script_type.is_valid()) {
	    ...
	    for (uint32_t i = 0; i < property_list.size(); i++) {
			...
			const OScriptParser::DataType property_type = type_from_property(property_info, false, false);

			p_identifier->set_datatype(property_type);
			p_identifier->source = OScriptParser::IdentifierNode::MEMBER_VARIABLE;
			return;
		}

	    MethodInfo method_info = GDE::Script::get_method_info(script_type, p_identifier->name);
	    ...
	    for (uint32_t i = 0 ; i < signal_list.size(); i++) {
			...
			const OScriptParser::DataType signal_type = make_signal_type(signal_info);
	    ...
	    const Dictionary constant_map = script_type->get_script_constant_map();
		if (constant_map.has(p_identifier->name)) {
			const Variant& constant = constant_map[p_identifier->name];
	...
	if (class_exists(native)) {
		...
		MethodInfo method_info;
		if (GDE::ClassDB::has_property(native, name)) {
			StringName getter_name = GDE::ClassDB::get_property_getter(native, name);
		    // todo: this doesn't work in GDE for native classes as the MethodBind are not accessible.
			// MethodBind *getter = ClassDB::get_method(native, getter_name);
			// if (getter != nullptr) {
			// 	bool has_setter = GDE::ClassDB::get_property_setter(native, name) != StringName();
			// 	p_identifier->set_datatype(type_from_property(GodotHelpers::MethodBind::get_return_info(getter), false, !has_setter));
			// 	p_identifier->source = OScriptParser::IdentifierNode::INHERITED_VARIABLE;
			// }
		    if (!getter_name.is_empty()) {
		        MethodInfo method;
		        if (GDE::ClassDB::get_method_info(native, getter_name, method)) {
		            bool has_setter = GDE::ClassDB::get_property_setter(native, name) != StringName();
		            p_identifier->set_datatype(type_from_property(method.return_val, false, !has_setter));
		            p_identifier->source = OScriptParser::IdentifierNode::INHERITED_VARIABLE;
		        }
		    }
			return;
		}
		if (GDE::ClassDB::get_method_info(native, name, method_info)) {
			// Method is callable.
			p_identifier->set_datatype(make_callable_type(method_info));
			p_identifier->source = OScriptParser::IdentifierNode::INHERITED_VARIABLE;
			return;
		}
		if (GDE::ClassDB::get_signal(native, name, method_info)) {
			// Signal is a type too.
			p_identifier->set_datatype(make_signal_type(method_info));
			p_identifier->source = OScriptParser::IdentifierNode::INHERITED_VARIABLE;
			return;
		}
		if (GDE::ClassDB::has_enum(native, name)) {
		...
		int64_t int_constant = GDE::ClassDB::get_integer_constant(native, name, valid);
		if (valid) {
			p_identifier->is_constant = true;
			...
			StringName enum_name = GDE::ClassDB::get_integer_constant_enum(native, name);
			if (enum_name != StringName()) {
...
void OScriptAnalyzer::reduce_identifier_from_base_set_class(OScriptParser::IdentifierNode* p_identifier, OScriptParser::DataType p_identifier_datatype) {
    ERR_FAIL_NULL(p_identifier);
    p_identifier->set_datatype(p_identifier_datatype);
    Error err = OK;
    Ref<OScript> scr = get_depended_shallow_script(p_identifier_datatype.script_path, err);
    if (err) {
...
void OScriptAnalyzer::reduce_lambda(OScriptParser::LambdaNode* p_lambda) {
    // Lambda is always a Callable.
    OScriptParser::DataType lambda_type;
    lambda_type.type_source = OScriptParser::DataType::ANNOTATED_INFERRED;
    lambda_type.kind = OScriptParser::DataType::BUILTIN;
...
void OScriptAnalyzer::reduce_literal(OScriptParser::LiteralNode* p_literal) {
...
void OScriptAnalyzer::reduce_preload(OScriptParser::PreloadNode* p_preload) {
    if (!p_preload->path) {
		return;
	}
	...
	if (p_preload->path->reduced_value.get_type() != Variant::STRING) {
	    ...
	    if (!ResourceLoader::get_singleton()->exists(p_preload->resolved_path)) {
	        if (FileAccess::file_exists(p_preload->resolved_path)) {
			...
			if (res_type == "GDScript") {
			    ...
			    // todo: support async load
				// Error err = OK;
				// p_preload->resource = GodotHelpers::ResourceLoader::load(p_preload->resolved_path, res_type, ResourceFormatLoader::CACHE_MODE_REUSE, &err);
				// if (err == ERR_BUSY) {
				// 	p_preload->resource = ResourceLoader::ensure_resource_ref_override_for_outer_load(p_preload->resolved_path, res_type);
				// }
			    p_preload->resource = ResourceLoader::get_singleton()->load(p_preload->resolved_path, res_type, ResourceLoader::CACHE_MODE_REUSE);
...
void OScriptAnalyzer::reduce_self(OScriptParser::SelfNode* p_self) {
    p_self->is_constant = false;
    p_self->set_datatype(type_from_metatype(parser->current_class->get_datatype()));
    mark_lambda_use_self();
}

void OScriptAnalyzer::reduce_subscript(OScriptParser::SubscriptNode* p_subscript, bool p_can_be_pseudo_type) {
    if (p_subscript->base == nullptr) {
		return;
	}

    if (p_subscript->base->type == OScriptParser::Node::IDENTIFIER) {
		reduce_identifier(static_cast<OScriptParser::IdentifierNode*>(p_subscript->base), true);
	} else if (p_subscript->base->type == OScriptParser::Node::SUBSCRIPT) {
		reduce_subscript(static_cast<OScriptParser::SubscriptNode*>(p_subscript->base), true);
	} else {
		reduce_expression(p_subscript->base);
	}

	OScriptParser::DataType result_type;
	...
	if (p_subscript->is_attribute) {
		...
		OScriptParser::DataType base_type = p_subscript->base->get_datatype();
		bool valid = false;

		// If the base is a metatype, use the analyzer instead.
		if (p_subscript->base->is_constant && !base_type.is_meta_type) {
			// If the base is a Script, use the analyzer instead.
			bool base_is_script = false;
			if (p_subscript->base->reduced_value.get_type() == Variant::OBJECT) {
				Ref<OScript> oscript = Object::cast_to<OScript>(p_subscript->base->reduced_value.get_validated_object());
				if (oscript.is_valid()) {
					base_is_script = true;
					// Makes a metatype from a constant Script, since `base_type` is not a metatype.
					OScriptParser::DataType base_type_meta = type_from_variant(oscript, p_subscript);
					// First try to reduce the attribute from the metatype.
					reduce_identifier_from_base(p_subscript->attribute, &base_type_meta);
					OScriptParser::DataType attr_type = p_subscript->attribute->get_datatype();
					...
					if (!valid) {
						...
						p_subscript->attribute->set_datatype(OScriptParser::DataType());
					}
				}
			}
		...
		if (valid) {
			...
			result_type.kind = OScriptParser::DataType::VARIANT;
			...
			if (base_type.is_variant() && base_type.is_hard_type() && base_type.is_meta_type && base_type.is_pseudo_type) {
				...
				String enum_name;
				if (p_subscript->base->type == OScriptParser::Node::IDENTIFIER) {
					enum_name = String(static_cast<OScriptParser::IdentifierNode*>(p_subscript->base)->name) + ENUM_SEPARATOR + String(p_subscript->attribute->name);
				}
				if (GDE::CoreConstants::is_global_enum(enum_name)) {
			...
			if (attr_type.is_set()) {
				if (base_type.builtin_type == Variant::DICTIONARY && base_type.has_container_element_types()) {
					...
					if (base_type.has_container_element_type(1)) {
						...
						result_type.kind = OScriptParser::DataType::VARIANT;
						result_type.type_source = OScriptParser::DataType::UNDETECTED;
					}
				} else {
					valid = !attr_type.is_pseudo_type || p_can_be_pseudo_type;
					result_type = attr_type;
				...
				valid = base_type.kind != OScriptParser::DataType::BUILTIN;
                #ifdef DEBUG_ENABLED
				if (valid) {
					parser->push_warning(p_subscript, OScriptWarning::UNSAFE_PROPERTY_ACCESS, p_subscript->attribute->name, base_type.to_string());
				}
                #endif // DEBUG_ENABLED
				result_type.kind = OScriptParser::DataType::VARIANT;
				mark_node_unsafe(p_subscript);
		...
		if (!valid) {
			...
			if (!p_can_be_pseudo_type && (attr_type.is_pseudo_type || result_type.is_pseudo_type)) {
				push_error(vformat(R"(Type "%s" in base "%s" cannot be used on its own.)",
				    p_subscript->attribute->name, type_from_metatype(base_type).to_string()), p_subscript->attribute);
			} else {
				push_error(vformat(R"(Cannot find member "%s" in base "%s".)",
				    p_subscript->attribute->name, type_from_metatype(base_type).to_string()), p_subscript->attribute);
			}
			result_type.kind = OScriptParser::DataType::VARIANT;
		}
		...
		if (p_subscript->base->is_constant && p_subscript->index->is_constant) {
			...
			OScriptParser::DataType base_type = p_subscript->base->get_datatype();
			OScriptParser::DataType index_type = p_subscript->index->get_datatype();

			if (base_type.is_variant()) {
				result_type.kind = OScriptParser::DataType::VARIANT;
				mark_node_unsafe(p_subscript);
			} else {
				if (base_type.kind == OScriptParser::DataType::BUILTIN && !index_type.is_variant()) {
					// Check if indexing is valid.
					bool error = index_type.kind != OScriptParser::DataType::BUILTIN && base_type.builtin_type != Variant::DICTIONARY;
					if (!error) {
						switch (base_type.builtin_type) {
							...
							case Variant::DICTIONARY: {
						        if (base_type.has_container_element_type(0)) {
						            OScriptParser::DataType key_type = base_type.get_container_element_type(0);
						...
						if (error) {
							push_error(vformat(R"(Invalid index type "%s" for a base of type "%s".)",
							    index_type.to_string(), base_type.to_string()), p_subscript->index);
						}
					}
				} else if (base_type.kind != OScriptParser::DataType::BUILTIN && !index_type.is_variant()) {
					if (index_type.builtin_type != Variant::STRING && index_type.builtin_type != Variant::STRING_NAME) {
						push_error(vformat(R"(Only "String" or "StringName" can be used as index for type "%s", but received "%s".)",
						    base_type.to_string(), index_type.to_string()), p_subscript->index);
					}
				}

				// Check resulting type if possible.
				result_type.builtin_type = Variant::NIL;
				result_type.kind = OScriptParser::DataType::BUILTIN;
				result_type.type_source = base_type.is_hard_type() ? OScriptParser::DataType::ANNOTATED_INFERRED : OScriptParser::DataType::INFERRED;

				if (base_type.kind != OScriptParser::DataType::BUILTIN) {
...
void OScriptAnalyzer::reduce_ternary_op(OScriptParser::TernaryOpNode* p_ternary_op, bool p_is_root) {
    ...
    if (p_ternary_op->condition && p_ternary_op->condition->is_constant && p_ternary_op->true_expr->is_constant &&
        p_ternary_op->false_expr && p_ternary_op->false_expr->is_constant) {
    ...
    OScriptParser::DataType true_type;
    if (p_ternary_op->true_expr) {
        true_type = p_ternary_op->true_expr->get_datatype();
    } else {
        true_type.kind = OScriptParser::DataType::VARIANT;
    }

    OScriptParser::DataType false_type;
    if (p_ternary_op->false_expr) {
        false_type = p_ternary_op->false_expr->get_datatype();
    } else {
        false_type.kind = OScriptParser::DataType::VARIANT;
    }
    ...
    if (true_type.is_variant() || false_type.is_variant()) {
        ...
        if (!is_type_compatible(true_type, false_type)) {
            ...
            if (!is_type_compatible(false_type, true_type)) {
                result.kind = OScriptParser::DataType::VARIANT;
                #ifdef DEBUG_ENABLED
                parser->push_warning(p_ternary_op, OScriptWarning::INCOMPATIBLE_TERNARY);
                #endif // DEBUG_ENABLED
            }
        }
    }
    ...
    result.type_source = true_type.is_hard_type() && false_type.is_hard_type()
        ? OScriptParser::DataType::ANNOTATED_INFERRED
        : OScriptParser::DataType::INFERRED;

    p_ternary_op->set_datatype(result);
}

void OScriptAnalyzer::reduce_type_test(OScriptParser::TypeTestNode* p_type_test) {
    OScriptParser::DataType result;
    result.type_source = OScriptParser::DataType::ANNOTATED_EXPLICIT;
    result.kind = OScriptParser::DataType::BUILTIN;
    result.builtin_type = Variant::BOOL;
    ...
    OScriptParser::DataType operand_type = p_type_test->operand->get_datatype();
    OScriptParser::DataType test_type = type_from_metatype(resolve_datatype(p_type_test->test_type));
...
void OScriptAnalyzer::reduce_unary_op(OScriptParser::UnaryOpNode* p_unary_op) { // NOLINT
    ...
    OScriptParser::DataType operand_type = p_unary_op->operand->get_datatype();
    if (p_unary_op->operand->is_constant) {
        p_unary_op->is_constant = true;
        p_unary_op->reduced_value = GDE::Variant::evaluate(p_unary_op->variant_op, p_unary_op->operand->reduced_value, Variant());
    ...
    if (operand_type.is_variant()) {
        ...
        if (!valid) {
            push_error(vformat(R"(Invalid operand of type "%s" for unary operator "%s".)",
                operand_type.to_string(), GDE::Variant::get_operator_name(p_unary_op->variant_op)), p_unary_op);
        }
    }
...
Array OScriptAnalyzer::make_array_from_element_datatype(const OScriptParser::DataType& p_element_datatype, const OScriptParser::Node* p_source_node) {
    Array array;

    if (p_element_datatype.builtin_type == Variant::OBJECT) {
        Ref<Script> script_type = p_element_datatype.script_type;
        if (p_element_datatype.kind == OScriptParser::DataType::CLASS && script_type.is_null()) {
            Error err = OK;
            Ref<OScript> scr = get_depended_shallow_script(p_element_datatype.script_path, err);
            if (err) {
...
Dictionary OScriptAnalyzer::make_dictionary_from_element_datatype(const OScriptParser::DataType& p_key_element_datatype, const OScriptParser::DataType& p_value_element_datatype, const OScriptParser::Node* p_source_node) {
    Dictionary dictionary;
    StringName key_name;
    Variant key_script;
    ...
    if (p_key_element_datatype.builtin_type == Variant::OBJECT) {
        ...
        if (p_key_element_datatype.kind == OScriptParser::DataType::CLASS && script_type.is_null()) {
            Error err = OK;
            Ref<OScript> scr = get_depended_shallow_script(p_key_element_datatype.script_path, err);
            if (err) {
    ...
    if (p_value_element_datatype.builtin_type == Variant::OBJECT) {
        ...
        if (p_value_element_datatype.kind == OScriptParser::DataType::CLASS && script_type.is_null()) {
            Error err = OK;
            Ref<OScript> scr = get_depended_shallow_script(p_value_element_datatype.script_path, err);
            if (err) {
...
Variant OScriptAnalyzer::make_dictionary_reduced_value(OScriptParser::DictionaryNode* p_dictionary, bool& is_reduced) {
        Dictionary dictionary = p_dictionary->get_datatype().has_container_element_types()
                ? make_dictionary_from_element_datatype(
                    ...
                    p_dictionary->get_datatype().get_container_element_type_or_variant(1))
                : Dictionary();
...
Variant OScriptAnalyzer::make_call_reduced_value(OScriptParser::CallNode* p_call, bool& is_reduced) {
    if (p_call->get_callee_type() == OScriptParser::Node::IDENTIFIER) {
        ...
        if (p_call->function_name == StringName("Array")) {
            type = Variant::ARRAY;
        } else if (p_call->function_name == StringName("Dictionary")) {
        ...
        Variant result;
        GDExtensionCallError ce = GDE::Variant::construct(type, result, argptrs, args.size());
        if (ce.error) {
...
bool OScriptAnalyzer::is_type_compatible(const OScriptParser::DataType& p_target, const OScriptParser::DataType& p_source, bool p_allow_implicit_conversion, const OScriptParser::Node* p_source_node) {
    #ifdef DEBUG_ENABLED
    if (p_source_node) {
        if (p_target.kind == OScriptParser::DataType::ENUM) {
            if (p_source.kind == OScriptParser::DataType::BUILTIN && p_source.builtin_type == Variant::INT) {
                parser->push_warning(p_source_node, OScriptWarning::INT_AS_ENUM_WITHOUT_CAST);
            }
        }
    }
    #endif // DEBUG_ENABLED
...
void OScriptAnalyzer::is_shadowing(OScriptParser::IdentifierNode* p_identifier, const String& p_context, const bool p_in_local_scope) {
    const StringName &name = p_identifier->name;
	{
        ...
        if (GDE::Variant::has_utility_function(name)) {
			...
			parser->push_warning(p_identifier, OScriptWarning::SHADOWED_GLOBAL_IDENTIFIER, p_context, name, "native class");
			return;
		} else if (ScriptServer::is_global_class(name)) {
			String class_path = ScriptServer::get_global_class_path(name).get_file();
			parser->push_warning(p_identifier, OScriptWarning::SHADOWED_GLOBAL_IDENTIFIER, p_context, name, vformat(R"(global class defined in "%s")", class_path));
			return;
	...
	const OScriptParser::DataType current_class_type = parser->current_class->get_datatype();
	if (p_in_local_scope) {
		OScriptParser::ClassNode* base_class = current_class_type.class_type;
		if (base_class != nullptr) {
		...
		while (base_class != nullptr) {
			if (base_class->has_member(name)) {
				...
				parser->push_warning(p_identifier, OScriptWarning::SHADOWED_VARIABLE_BASE_CLASS, p_context,
				    p_identifier->name, base_class->get_member(name).get_type_name(),
				    itos(base_class->get_member(name).get_script_node_id()), base_class_name);
				return;
			}
			base_class = base_class->base_type.class_type;
		}
	...
	while (native_base_class != StringName()) {
		...
		if (ClassDB::class_has_method(native_base_class, name, true)) {
			parser->push_warning(p_identifier, OScriptWarning::SHADOWED_VARIABLE_BASE_CLASS, p_context, p_identifier->name, "method", native_base_class);
			return;
		} else if (GDE::ClassDB::has_signal(native_base_class, name, true)) {
			parser->push_warning(p_identifier, OScriptWarning::SHADOWED_VARIABLE_BASE_CLASS, p_context, p_identifier->name, "signal", native_base_class);
			return;
		} else if (GDE::ClassDB::has_property(native_base_class, name, true)) {
			parser->push_warning(p_identifier, OScriptWarning::SHADOWED_VARIABLE_BASE_CLASS, p_context, p_identifier->name, "property", native_base_class);
			return;
		} else if (GDE::ClassDB::has_integer_constant(native_base_class, name, true)) {
			parser->push_warning(p_identifier, OScriptWarning::SHADOWED_VARIABLE_BASE_CLASS, p_context, p_identifier->name, "constant", native_base_class);
			return;
		} else if (GDE::ClassDB::has_enum(native_base_class, name, true)) {
			parser->push_warning(p_identifier, OScriptWarning::SHADOWED_VARIABLE_BASE_CLASS, p_context, p_identifier->name, "enum", native_base_class);
			return;
		}
...
void OScriptAnalyzer::push_error(const String& p_message, const OScriptParser::Node* p_origin) {
    mark_node_unsafe(p_origin);
    parser->push_error(p_message, p_origin);
}

void OScriptAnalyzer::mark_node_unsafe(const OScriptParser::Node* p_node) {
    #ifdef DEBUG_ENABLED
    if (p_node == nullptr) {
        return;
    }
    parser->unsafe_nodes.insert(p_node->script_node_id);
    #endif
}

void OScriptAnalyzer::update_const_expression_builtin_type(OScriptParser::ExpressionNode* p_expression, const OScriptParser::DataType& p_type, const char* p_usage, bool p_is_cast) {
    if (p_expression->get_datatype() == p_type) {
		return;
	}

	if (p_type.kind != OScriptParser::DataType::BUILTIN && p_type.kind != OScriptParser::DataType::ENUM) {
		return;
	}

	OScriptParser::DataType expression_type = p_expression->get_datatype();
	bool is_enum_cast = p_is_cast && p_type.kind == OScriptParser::DataType::ENUM && p_type.is_meta_type == false && expression_type.builtin_type == Variant::INT;
	...
	OScriptParser::DataType value_type = type_from_variant(p_expression->reduced_value, p_expression);
    ...
    #ifdef DEBUG_ENABLED
	if (p_type.kind == OScriptParser::DataType::ENUM && value_type.builtin_type == Variant::INT && !enum_has_value(p_type, p_expression->reduced_value)) {
		parser->push_warning(p_expression, OScriptWarning::INT_AS_ENUM_WITHOUT_MATCH, p_usage, p_expression->reduced_value.stringify(), p_type.to_string());
	}
    #endif // DEBUG_ENABLED
    ...
    GDExtensionCallError call_error = GDE::Variant::construct(p_type.builtin_type, converted_to, &converted_from, 1);
	if (call_error.error) {
	...
	if (p_type.builtin_type == Variant::INT && value_type.builtin_type == Variant::FLOAT) {
		parser->push_warning(p_expression, OScriptWarning::NARROWING_CONVERSION);
	}
    #endif // DEBUG_ENABLED
...
void OScriptAnalyzer::update_array_literal_element_type(OScriptParser::ArrayNode* p_array, const OScriptParser::DataType& p_element_type) {
    OScriptParser::DataType expected_type = p_element_type;
    expected_type.container_element_types.clear(); // Nested types (like `Array[Array[int]]`) are not currently supported.

    for (int i = 0; i < p_array->elements.size(); i++) {
        OScriptParser::ExpressionNode* element_node = p_array->elements[i];
        if (element_node->is_constant) {
            update_const_expression_builtin_type(element_node, expected_type, "include");
        }

        const OScriptParser::DataType& actual_type = element_node->get_datatype();
        ...
        if (!is_type_compatible(expected_type, actual_type, true, p_array)) {
            ...
            push_error(vformat(R"(Cannot have an element of type "%s" in an array of type "Array[%s]".)",
                actual_type.to_string(), expected_type.to_string()), element_node);
            return;
        }
    }

    OScriptParser::DataType array_type = p_array->get_datatype();
...
// This function determines which type is that (if any).
void OScriptAnalyzer::update_dictionary_literal_element_type(OScriptParser::DictionaryNode* p_dictionary, const OScriptParser::DataType& p_key_element_type, const OScriptParser::DataType& p_value_element_type) {
    OScriptParser::DataType expected_key_type = p_key_element_type;
	OScriptParser::DataType expected_value_type = p_value_element_type;
	...
	for (int i = 0; i < p_dictionary->elements.size(); i++) {
		OScriptParser::ExpressionNode* key_element_node = p_dictionary->elements[i].key;
		if (key_element_node->is_constant) {
			update_const_expression_builtin_type(key_element_node, expected_key_type, "include");
		}

		const OScriptParser::DataType& actual_key_type = key_element_node->get_datatype();
		if (actual_key_type.has_no_type() || actual_key_type.is_variant() || !actual_key_type.is_hard_type()) {
			...
			if (is_type_compatible(actual_key_type, expected_key_type)) {
				...
				push_error(vformat(R"(Cannot have a key of type "%s" in a dictionary of type "Dictionary[%s, %s]".)",
				    actual_key_type.to_string(), expected_key_type.to_string(), expected_value_type.to_string()), key_element_node);
				return;
			}
		}

		OScriptParser::ExpressionNode* value_element_node = p_dictionary->elements[i].value;
		if (value_element_node->is_constant) {
			update_const_expression_builtin_type(value_element_node, expected_value_type, "include");
		}
		const OScriptParser::DataType& actual_value_type = value_element_node->get_datatype();
		if (actual_value_type.has_no_type() || actual_value_type.is_variant() || !actual_value_type.is_hard_type()) {
			...
			if (is_type_compatible(actual_value_type, expected_value_type)) {
				...
				push_error(vformat(R"(Cannot have a value of type "%s" in a dictionary of type "Dictionary[%s, %s]".)",
				    actual_value_type.to_string(), expected_key_type.to_string(), expected_value_type.to_string()), value_element_node);
				return;
			}
		}
	}

	OScriptParser::DataType dictionary_type = p_dictionary->get_datatype();
...
void OScriptAnalyzer::validate_call_arg(const List<OScriptParser::DataType>& p_par_types, int p_default_args_count, bool p_is_vararg, const OScriptParser::CallNode* p_call) {
    if (p_call->arguments.size() < p_par_types.size() - p_default_args_count) {
		push_error(vformat(R"*(Too few arguments for "%s()" call. Expected at least %d but received %d.)*",
		    p_call->function_name, p_par_types.size() - p_default_args_count, p_call->arguments.size()), p_call);
	}
	if (!p_is_vararg && p_call->arguments.size() > p_par_types.size()) {
		push_error(vformat(R"*(Too many arguments for "%s()" call. Expected at most %d but received %d.)*",
		    p_call->function_name, p_par_types.size(), p_call->arguments.size()), p_call->arguments[p_par_types.size()]);
	}

	List<OScriptParser::DataType>::ConstIterator par_itr = p_par_types.begin();
	for (int i = 0; i < p_call->arguments.size(); ++par_itr, ++i) {
		if (i >= p_par_types.size()) {
			// Already on vararg place.
			break;
		}
		OScriptParser::DataType par_type = *par_itr;
		...
		if (arg_type.is_variant() || !arg_type.is_hard_type()) {
			...
			if (!(par_type.is_hard_type() && par_type.is_variant())) {
				...
				parser->push_warning(p_call->arguments[i], OScriptWarning::UNSAFE_CALL_ARGUMENT,
				    itos(i + 1), "function", p_call->function_name, par_type.to_string(), arg_type.to_string_strict());
			}
            #endif // DEBUG_ENABLED
			...
			if (!is_type_compatible(arg_type, par_type)) {
				...
				mark_node_unsafe(p_call);
				parser->push_warning(p_call->arguments[i], OScriptWarning::UNSAFE_CALL_ARGUMENT, itos(i + 1),
				    "function", p_call->function_name, par_type.to_string(), arg_type.to_string_strict());
            #endif // DEBUG_ENABLED
			}
        #ifdef DEBUG_ENABLED
		} else if (par_type.kind == OScriptParser::DataType::BUILTIN && par_type.builtin_type == Variant::INT &&
		    arg_type.kind == OScriptParser::DataType::BUILTIN && arg_type.builtin_type == Variant::FLOAT) {
			parser->push_warning(p_call->arguments[i], OScriptWarning::NARROWING_CONVERSION, p_call->function_name);
        #endif // DEBUG_ENABLED
		}
	}
}

void OScriptAnalyzer::validate_call_arg(const MethodInfo& p_method, const OScriptParser::CallNode* p_call) {
    List<OScriptParser::DataType> arg_types;
...
void OScriptAnalyzer::downgrade_node_type_source(OScriptParser::Node* p_node) {
    OScriptParser::IdentifierNode* identifier = nullptr;
    if (p_node->type == OScriptParser::Node::IDENTIFIER) {
        identifier = static_cast<OScriptParser::IdentifierNode*>(p_node);
    } else if (p_node->type == OScriptParser::Node::SUBSCRIPT) {
        OScriptParser::SubscriptNode* subscript = static_cast<OScriptParser::SubscriptNode*>(p_node);
        if (subscript->is_attribute) {
    ...
    OScriptParser::Node* source = nullptr;
    ...
    OScriptParser::DataType datatype;
    datatype.kind = OScriptParser::DataType::VARIANT;
...
Error OScriptAnalyzer::resolve_interface() {
    resolve_class_interface(parser->head, true);
    return parser->errors.is_empty() ? OK : ERR_PARSE_ERROR;
}

Error OScriptAnalyzer::resolve_body() {
    resolve_class_body(parser->head, true);
    #ifdef DEBUG_ENABLED
    // Apply after all `@warning_ignore` have been resolved and applied
    parser->apply_pending_warnings();
    #endif
    return parser->errors.is_empty() ? OK : ERR_PARSE_ERROR;
}

Error OScriptAnalyzer::resolve_dependencies() {
    for (KeyValue<String, Ref<OScriptParserRef>>& E : parser->depended_parsers) {
        if (E.value.is_null()) {
            return ERR_PARSE_ERROR;
        }
        E.value->raise_status(OScriptParserRef::INHERITANCE_SOLVED);
    }
    return parser->errors.is_empty() ? OK : ERR_PARSE_ERROR;
}

Error OScriptAnalyzer::analyze() {
    parser->errors.clear();
...
Variant OScriptAnalyzer::make_variable_default_value(OScriptParser::VariableNode* p_variable) {
    ...
    if (p_variable->initializer) {
        ...
        OScriptParser::DataType datatype = p_variable->get_datatype();
        if (datatype.is_hard_type()) {
            if (datatype.kind == OScriptParser::DataType::BUILTIN && datatype.builtin_type != Variant::OBJECT) {
                if (datatype.builtin_type == Variant::ARRAY && datatype.has_container_element_type(0)) {
                    ...
                    OScriptParser::DataType key = datatype.get_container_element_type_or_variant(0);
                    OScriptParser::DataType value = datatype.get_container_element_type_or_variant(1);
                    result = make_dictionary_from_element_datatype(key, value);
                } else {
                    GDE::VariantInternal::initialize(&result, datatype.builtin_type);
                }
            } else if (datatype.kind == OScriptParser::DataType::ENUM) {
                result = 0;
            }
        }
    }
...
bool OScriptAnalyzer::check_type_compatibility(const OScriptParser::DataType& p_target, const OScriptParser::DataType& p_source, bool p_allow_implicit_conversion, const OScriptParser::Node* p_source_node) {
	...
	if (p_target.kind == OScriptParser::DataType::VARIANT) {
		// Variant can receive anything.
		return true;
	}

	if (p_source.kind == OScriptParser::DataType::VARIANT) {
		// TODO: This is acceptable but unsafe. Make sure unsafe line is set.
		return true;
	}

	if (p_target.kind == OScriptParser::DataType::BUILTIN) {
		bool valid = p_source.kind == OScriptParser::DataType::BUILTIN && p_target.builtin_type == p_source.builtin_type;
		if (!valid && p_allow_implicit_conversion) {
			valid = Variant::can_convert_strict(p_source.builtin_type, p_target.builtin_type);
		}

		if (!valid && p_target.builtin_type == Variant::INT && p_source.kind == OScriptParser::DataType::ENUM && !p_source.is_meta_type) {
	...
	if (p_target.kind == OScriptParser::DataType::ENUM) {
		...
		if (p_source.kind == OScriptParser::DataType::ENUM) {
	...
	if (p_source.kind == OScriptParser::DataType::BUILTIN && p_source.builtin_type == Variant::NIL) {
	...
	const OScriptParser::ClassNode* src_class = nullptr;

	switch (p_source.kind) {
		case OScriptParser::DataType::NATIVE: {
		    if (p_target.kind != OScriptParser::DataType::NATIVE) {
		        // Non-native class cannot be supertype of native.
		        return false;
		    }
		    if (p_source.is_meta_type) {
		        src_native = OScriptNativeClass::get_class_static();
		    } else {
		...
		case OScriptParser::DataType::SCRIPT: {
		    if (p_target.kind == OScriptParser::DataType::CLASS) {
		        // A script type cannot be a subtype of a OScript class.
		        return false;
		    }
		    ...
		    if (p_source.is_meta_type) {
		        src_native = p_source.script_type->get_class();
		    } else {
		...
		case OScriptParser::DataType::CLASS: {
		    if (p_source.is_meta_type) {
		        src_native = OScript::get_class_static();
		    } else {
		        src_class = p_source.class_type;
		        const OScriptParser::ClassNode *base = src_class;
		        while (base->base_type.kind == OScriptParser::DataType::CLASS) {
	...
	switch (p_target.kind) {
		case OScriptParser::DataType::NATIVE: {
			if (p_target.is_meta_type) {
				return ClassDB::is_parent_class(src_native, OScriptNativeClass::get_class_static());
			}
			return ClassDB::is_parent_class(src_native, p_target.native_type);
		}
		case OScriptParser::DataType::SCRIPT: {
		    if (p_target.is_meta_type) {
		        return ClassDB::is_parent_class(src_native, p_target.script_type->get_class());
		    }
		...
		case OScriptParser::DataType::CLASS: {
		    if (p_target.is_meta_type) {
		        return ClassDB::is_parent_class(src_native, OScript::get_class_static());
		    }
...
OScriptParser::DataType OScriptAnalyzer::type_from_metatype(const OScriptParser::DataType& p_type) {
    OScriptParser::DataType result = p_type;
    result.is_meta_type = false;
    result.is_pseudo_type = false;

    if (p_type.kind == OScriptParser::DataType::ENUM) {
...
bool OScriptAnalyzer::class_exists(const StringName& p_class) {
    return ClassDB::class_exists(p_class) && GDE::ClassDB::is_class_exposed(p_class);
}

OScriptAnalyzer::OScriptAnalyzer(OScriptParser* p_parser) {
    parser = p_parser;
}
